...
toadd = c(  "age_BL", "commonSex", 'race',  'duration_yrs', "updrs_totscore", 'AsynStatus', 'LEDD', 'subgroup' )
...
######################################################################
dxform=" commonEdu +  age_BL + commonSex + brainVolume_BL "
raneffstr = " " 
dxform=paste( raneffstr, " + noise + ssim + T1Hier_evratio + age_BL + commonSex " )
dxform=paste( raneffstr, " + age_BL + commonSex " )
mydog = ppmitrim[ subtyper::fs(ppmitrim$T1Hier_resnetGrade > 0.2), ]
dim(mydog)
...
###################
mydogU=mydog[ !is.na(mydog$TDX) & mydog$T1Hier_resnetGrade > 1.02, ]
dxglist=list()
dxdfT1=data.frame()
...
for ( cog in  cogs[ -grep("tot",cogs)]  ) {
  for ( voi in nmstotest ) {
    ...
    cogformcommon=paste( cog, "  ~  ",motnameu,"+ duration_yrs + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + TDX " )
    cogformcommon=paste( cog, "  ~  ",motnameu,"+ age_BL + commonSex + TDX " )
    temp2 = mydogp[ !is.na( mydogp[,voi]) & !( mydogp$TDX %in% c("CN-","CN+","ProSp-", "ProSp+" ) ), ]
