---
title: "Human curation for multi-view MRI in PPMI"
author:
  - Brian B. Avants^1,2^
  - Lino Becerra^1^
  - Leon Fonville^3,†^
...
if ( ! exists("clin2b") ) {
  ...
  if ( ! exists("pymf") ) {
    ...
    grep("102529-20211014", pym$filename)
    ...
    grep("102529-20211014", pymf$filename)
    rm(pym)
    }
  if ( ! exists("adni") ) 
  ...
  agenames=c("AGE","Age","age","subjectAge","PTAGE")
  ...
  grep("102529", ppmidemog0$PATNO)
  grep("102529", demog$PATNO)
  clin2b = merge_ppmi_imaging_clinical_demographic_data(demog, ppmidemog0, pymf, pymversion=myrez, saa )
  # add tracker
...
ppmi[ ppmi$studyName=='ADNI', 'age']=ppmi[ ppmi$studyName=='ADNI', 'AGE']
...
rgThresh=0.2
srth=0.002
snrth=quantile(ppmi$snr,c(srth,1.0-srth),na.rm=T)
zz=read.csv( paste0( rdir, "ppmi500_ids_date.csv") )
zz$subjectIDdate=zz$subjectID
for ( k in 1:nrow(zz)) zz$subjectIDdate[k]=paste0(zz$subjectID[k],"-",zz$date[k])
psel = ppmi$subjectID %in% zz$subjectID  & ppmi$yearsbl==0
ppmitrim = ppmi[ psel, ]
zz$subjectID[ !(zz$subjectID %in% ppmi$subjectID) ]
table(zz$subjectID %in% ppmi$subjectID)
table(zz$subjectID %in% ppmitrim$subjectID)
table(ppmitrim$subjectID %in% zz$subjectID)
table( is.na( ppmitrim$DTI_dti_FD_mean))
ppmitrim$commonEdu = antsrimpute( ppmitrim$commonEdu )
table( ppmitrim$studyName )
ppmitrim[ is.na( ppmitrim$duration_yrs), 'duration_yrs' ]=0.0
...
```{r ggablly,echo=FALSE,eval=TRUE,fig.width=16,fig.height=10,message=FALSE,warning=FALSE}
...
```{r ggabllypost,echo=FALSE,eval=TRUE,fig.width=16,fig.height=10,message=FALSE,warning=FALSE}
...
ppmitrim0 = ppmitrim[ , c("subjectID","age",'commonSex', "hy", "duration_yrs","LEDD","moca",updnames, msnames, 'joinedDX','AsynStatus', "DXSubAsyn", "educ","race",'tau','ptau','abeta', 'yearsbl', 'EVENT_ID', 'date', lrrk2names,  'filename', 'brainVolume', t1vars, 'imaging_protocol', 'T1Hier_resnetGrade' ) ]
```

```{r ggabllypost2,echo=FALSE,eval=TRUE,fig.width=16,fig.height=10,message=FALSE,warning=FALSE}
...
toadd = c(  "age", "commonSex", 'race',  'duration_yrs', "updrs_totscore", 'AsynStatus', 'LEDD', 'subgroup','T1Hier_resnetGrade', 'EVENT_ID', 'subjectID','DTI_dti_FD_max','rsfMRI_fcnxpro129_FD_max' )
tblcols = unique( c( toadd, 'joinedDX'  ) )
ppmitrimU=highestQualityRepeat( ppmitrim[,tblcols], 'subjectID', 'EVENT_ID', 'T1Hier_resnetGrade')
joinedem = ppmitrimU[ , !( colnames(ppmitrimU) %in% 
  c('subjectID', 'EVENT_ID','T1Hier_resnetGrade','DTI_dti_FD_max','rsfMRI_fcnxpro129_FD_max' ))]
joinedem$hasDTI=!is.na(ppmitrimU[,'DTI_dti_FD_max'])
joinedem$hasfMRI=!is.na(ppmitrimU[,'rsfMRI_fcnxpro129_FD_max'])
...
##########
mytbl$joinedDX = as.character(mytbl$joinedDX  )
...
######################################################################
dxform=" commonEdu +  age + commonSex + brainVolume "
raneffstr = " " 
dxform=paste( raneffstr, " + noise + ssim + T1Hier_evratio + age + commonSex " )
dxform=paste( raneffstr, " + age + commonSex " )
ppmitrim$EVENT_ID = antsrimpute( ppmitrim$EVENT_ID, FUN=median)
mydog = ppmitrim[ subtyper::fs(ppmitrim$T1Hier_resnetGrade > rgThresh), ]
dim(mydog)
mydog=highestQualityRepeat(mydog,'commonID','EVENT_ID','T1Hier_resnetGrade')
dim(mydog)
...
###############################################
mydog$TDX = NA
mydog$TDX[ subtyper::fs(mydog$DXSubAsyn == 'CNNegative' ) ]='CN-'
mydog$TDX[ subtyper::fs(mydog$DXSubAsyn == 'CNPositive' ) ]='CN+'
mydog$TDX[ subtyper::fs(mydog$DXSubAsyn  == 'PDLRRK2Positive') ]=NA # 'PDLRRK2+'
mydog$TDX[ subtyper::fs(mydog$DXSubAsyn  == 'PDLRRK2Negative') ]=NA # 'PDLRRK2-'
mydog$TDX[ subtyper::fs(mydog$DXSubAsyn  == 'ProdromalSporadicPosMSA') ]=NA 
...
mydog$TDX[ subtyper::fs(mydog$DXSubAsyn  == 'ProdromalSporadicPositive') ]='ProSp+'
mydog$TDX[ subtyper::fs(mydog$DXSubAsyn  == 'ProdromalSporadicNegative') ]='ProSp-'
mydog$TDX <- fct_relevel(mydog$TDX, "CN-")

###############################################
# mydog$TDX = NA
# mydog$TDX[ subtyper::fs(mydog$joinedDX == 'CN' ) ]='CN'
# mydog$TDX[ subtyper::fs(mydog$joinedDX  == 'PDLRRK2') ]='PDLRRK2'
# mydog$TDX[ subtyper::fs(mydog$joinedDX  == 'ProdromalSporadicPosMSA') ]=NA 
# mydog$TDX[ subtyper::fs(mydog$joinedDX  == 'PDSporadic') ]='PDSp'
# mydog$TDX[ subtyper::fs(mydog$joinedDX  == 'ProdromalSporadic') ]='ProSp'
# mydog$TDX <- fct_relevel(mydog$TDX, "CN")
...
###################
mydogU=mydog[ !is.na(mydog$TDX) & mydog$T1Hier_resnetGrade > rgThresh, ]
dxglist=list()
dxdfT1=data.frame()
idpsU=idps[ -multigrep( c("alff","peraf"), idps ) ]
idpsU=idpsU[ -multigrep( c("fcnxpro134","fcnxpro129"), idpsU ) ]
idpsU=idpsU[ multigrep( c("_2_","LRAVG","Asym"), idpsU ) ]
for ( voi in nmstotest ) {
  n=nrow(dxdfT1)+1
  vois = paste0( voi, c("", "_delta" ) )
  ...
  bform = paste( voi, " ~ brainVolume+duration_yrs+ T1Hier_resnetGrade+", othdxform )
#  bform = paste( voi, " ~ brainVolume+commonEdu+duration_yrs+LEDD+", othdxform )
  bform = paste( voi, " ~ brainVolume+commonEdu+duration_yrs+", othdxform )
...
#  for ( x in  all.vars( as.formula(newform)) ) { print(x) ; print(table( is.na( temp2$duration_yrs ), temp2$studyName )) }
  mdl=lm( newform, data=temp2 )
  temp3 = temp2[ names(predict(mdl)),]
  mdl=lm( newform, data=temp3 )
  bmdl=lm( bform, data=temp3 )
  ...
  othercols=othercols[grep("TDX",othercols)]
  ...
  if ( any(  mycoffs[ multigrep( c("TDX"),rownames(mycoffs)), 4 ] < 0.005 ) ) {
#  if ( shorten_pymm_names(voi) %in%  thosetoviz ) {
    ...
    dxglist[[length(dxglist)+1]]=prplot( mdl, dxvar, titlestring=ttl, colorvar='commonSex', addpoints=1)+theme(legend.position = "none")
    print( dxglist[[length(dxglist)]] )
    # prplot( mdl, dxvar, 'commonSex', titlestring=ttl, colorvar='commonSex', addpoints=1)
#  mdlV=lme4::lm( newform, data=temp3 )
...
dxdfT1selcolsB = dxdfT1selcols[ -grep("p.",(dxdfT1selcols),fixed=TRUE ) ]
# Print the ztable object, ensuring it's treated as LaTeX code
...
cogs = getNamesFromDataframe( c("updrs","score"), mydog, exclusions=c("_delta","_BL",'totscore','updrs4') )
col2sbl = unique(c( idpst1, dtinamesasymlr, rsfnames, rsfnamescnx,  fdnames, cogs, nmstotest ))
mydogp = mydogU
mydogp$hy = factor( mydogp$hy )
stadfrsf=data.frame()
cogglist=list()
for ( cog in  cogs   ) {
  for ( voi in nmstotest ) {
    n = nrow(stadfrsf)+1
    maxy=5.0
    if ( antspymm_vartype( voi ) == "T1" ) motnameu="1"
...
#      maxy=3.0
    ...
    cogformcommon=paste( cog, "  ~  ",motnameu,"+ duration_yrs + LEDD + commonEdu + age + commonSex + brainVolume + TDX " )
    cogformcommon=paste( cog, "  ~  ",motnameu,"+ age + commonSex + TDX " )
    temp2 = mydogp[ !is.na( mydogp[,voi]) & !( mydogp$TDX %in% c("CN-","CN+","PDSp-", "PDSp+" ) ), ]
    temp2 = mydogp[ !is.na( mydogp[,voi]), ]
    ...
    bmdl=(lm( cogformcommon, data=temp2 ))
    mdl=(lm( newform, data=temp2 ))
    mycoffs = coefficients( summary( mdl ) )
    anvp = suppressMessages( anova(bmdl,mdl)$Pr[2] )
    othercols=grep(voix,rownames(mycoffs))
    ndf = mdl$df
    chnd = effectsize::t_to_d( mycoffs[ othercols, 't value'], rep(ndf,length(othercols)) )
    stadfrsf[n,'cog']=cog
    stadfrsf[n,'voi']=voix
...
#    if (  paste0(voix,"_",cog) %in% ptv  ) {
      ...
      temp2$TDX[ temp2$TDX == 'ProSp-']='pro-'
      temp2$TDX[ temp2$TDX == 'ProSp+']='pro+'
      temp2$TDX[ temp2$TDX == 'PDSp+']='sp+'
      temp2$TDX[ temp2$TDX == 'PDSp-']='sp-'
      gg2 = prplot( mdl, voix, 'TDX', titlestring=ttl, colorvar='TDX' )
      print(gg2)
...
```{r quicklmcogpost,echo=FALSE,eval=buildsci2,fig.width=10,fig.height=6,warning=FALSE,message=FALSE,fig.cap='Partial regression interaction plots show a representative region for each of the four UPDRS outcomes that we test.  Each panel demonstrates how different levels of the IDP (x-axis) lead to different amounts of change over time. For example, the slope of the relationship btween MD asymmetry in the internal capsule and change in UPDRS2 score increases over time with the largest differences occuring at the right side of the top right panel.  This indicates that subjects with higher levels of baseline MD asymmetry in this IDP progress more rapidly on this score.'}
#####################
sigth=0.005
# for ( cog in cogs ) 
mypvs = stadfrsf$anv
psel = subtyper::fs( p.adjust( mypvs, 'none' ) <= sigth )
table(psel)
qdfsig=stadfrsf[ psel,]
...
mycap='Table 7. Significant (fdr) IDP-UPDRS effects: T1w, DTI and rsfMRI'
...
maxy=3000.0
vois = c(
  "T1Hier_vol_LRAVG_inferior_parietaldktcortex",
  "rsfMRI_fcnxpro122_SomMotB_2_ContA",
  "DTI_mean_md.fornix_.cres..stria_terminalis_.can_not_be_resolved_with_current_resolution..Asym.jhu_icbm_labels_1mm")
vois = c(
#  "DTI_mean_md.fornix_.cres..stria_terminalis_.can_not_be_resolved_with_current_resolution..Asym.jhu_icbm_labels_1mm",
  "T1Hier_vol_LRAVG_inferior_parietaldktcortex",
 # "DTI_mean_md.anterior_limb_of_internal_capsule.Asym.jhu_icbm_labels_1mm"
  "T1Hier_vol_LRAVG_superior_parietaldktcortex"
  )
voisx=vois
...
cogv=paste0(cog,c("","_delta"))
raneffstr=''
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs + DTI_dti_FD_max+ DTI_dti_FD_mean  + LEDD + commonEdu + age + commonSex + brainVolume + ( yearsbl ) + ( yearsbl )  ")
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs + LEDD + commonEdu + age + commonSex + brainVolume + ( yearsbl ) + ( yearsbl )  ")
...
dt=read.csv( paste0( rdir, 'dti_with_b.csv' ) )
dt$fn=gsub("vizx_2024/viz_PPMI-","",dt$fn)
dt$fn=gsub(".csv","",dt$fn)
dt$subjectID=NA
for ( x in 1:nrow(dt)) dt$subjectID[x]=unlist(strsplit(dt$fn[x],'-'))[1]
dt500 = dt[ dt$subjectID %in% zz$subjectID, ]
length(unique(dt500$subjectID))
dt500g = dt[ dt$subjectID %in% zz$subjectID & dt$bvecnormround > 0.5 & dt$bvalmax > 1, ]
length(unique(dt500g$subjectID))
goodrawdt=unique( mm500$subjectID[mm500$hasdti] )
goodprodt=unique( ppmitrim$subjectID[  !is.na(ppmitrim$DTI_dti_FD_mean)] )
missingdt=goodrawdt[ !(goodrawdt %in% goodprodt) ]
mm500 = mm[ mm$subjectID %in% zz$subjectID, ]
mm=read.csv( paste0( rdir, 'matched_mm_data5.csv' ) )
mm500 = mm[ mm$subjectID %in% zz$subjectID, ]
mm500$hasdti = mm500$dtid1 != ""
