...
```{r ppmitrim,echo=FALSE,eval=!exists("ppmitrim"),fig.width=16,fig.height=10,message=FALSE,warning=FALSE}
library(GGally)
...
doadj=TRUE
ppmitrim = fill1col2another( ppmitrim, 'moca_BL', 'MOCA_bl' )
ppmitrim = fill1col2another( ppmitrim, 'moca', 'MOCA' )
```


```{r ppmitrimAdj,echo=FALSE,eval=doadj,fig.width=16,fig.height=10,message=FALSE,warning=FALSE}
ppmitrim = ppmitrim[ ppmitrim$mrimfg %in% c("GE","Philips","Siemens"), ]
nmstotest2=sample( antspymm_predictors( ppmitrim, TRUE, TRUE ) )
nmstotest2=nmstotest2[ -multigrep( c("alff","peraf","template","fcnxpro134","fcnxpro122"), nmstotest2)]
# adjust each of these vars
ppmitrim$isTrain=FALSE
dxsel = ppmitrim$joinedDX %in% c("CN")
#  'ProdromalLRRK2','ProdromalGBA','ProdromalSporadic','SWEDDSporadic' )
agsel = ppmitrim$age_BL >= 50 & ppmitrim$age_BL <= 70
...
#    } else myform = paste( x, " ~ studyName + mrimfg + mriMagneticFieldStrength ")
    } else myform = paste( x, " ~ studyName + mrimfg ")
    ppmitrim=adjustByCovariates( ppmitrim, myform, 'isTrain', TRUE, TRUE )
}
nmstotest2=paste0(nmstotest2,'_adjusted' )

oldsimnames = getNamesFromDataframe("PC",ppmitrim,exclusions='rsfMRI')
if ( length(oldsimnames) > 0 ) {
  ppmitrim=ppmitrim[,!(colnames(ppmitrim) %in% oldsimnames)]
}
mysim = antspymm_simlr( ppmitrim, ppmitrim$yearsbl==0, 
    energy='reg', nsimlr=20, covariates='1', myseed=808, doAsym=2, 
    returnidps=FALSE,
    exclusions=c("_delta","_BL", '122', '134' ), 
    # sparseness=0.95,
    inclusions='_adjusted',
    verbose=TRUE)
...
toadd = c(  "age_BL", "commonSex", 'race',  'duration_yrs', "updrs_totscore", 'AsynStatus', 'LEDD', 'subgroup', 'imaging_protocol', 'mrimfg', 'mean_striatum')
...
wsaa=which(names(joinedem)=='mean_striatum' )
names(joinedem)[wsaa]=tblcols[wsaa]='DAT.striatum'
...
selfmri=!is.na(  ppmi$rsfMRI_fcnxpro129_VisPeri_2_SalVentAttnA) & 
  !is.na(ppmi$rsfMRI_fcnxpro129_ContA_2_ContA) & 
  subtyper::fs(ppmi$rsfMRI_fcnxpro129_FD_mean <= 0.5)
ppmi$DXbasic = NA
...
idpsrsfmri = c(
  getNamesFromDataframe( c("rsfMRI_fcnxpro129","_2_"), ppmi, exclusions=xcl ),
  getNamesFromDataframe( c("rsfMRI_fcnxpro129","_falff"), ppmi, exclusions=xcl ) )
...
dxform=paste( raneffstr, " + T1Hier_resnetGrade + age_BL + commonSex " )
...
mydog = ppmitrim # [ subtyper::fs(ppmi$T1Hier_resnetGrade > 1.0), ]
...
#  subtyper::fs( ppmi$rsfMRI_fcnxpro129_n_outliers < 100 ) &
#  subtyper::fs( ppmi$rsfMRI_fcnxpro129_FD_max <= 0.5 )
...
rsfnamescnx = getNamesFromDataframe( c( 
    ...
    ppmi, exclusions=c(xcl,'efaul','Dors','SalVe','imbi')  )
...
```{r predefregions,echo=FALSE,eval=FALSE,warning=FALSE,message=FALSE,results='asis'}
...
```{r predefregions1,echo=FALSE,eval=FALSE,warning=FALSE,message=FALSE,results='asis'}
...
```{r predefregions2,echo=FALSE,eval=FALSE,warning=FALSE,message=FALSE,results='asis'}
...
```{r predefregions3,echo=FALSE,eval=FALSE,warning=FALSE,message=FALSE,results='asis'}
print(myregionszRSFz, type = "latex",size=4,caption=rsfcap,sidewaystable=FALSE,zebra=TRUE)
```

```{r quicklmdx,echo=FALSE,eval=!exists("mydogU"),fig.width=10,fig.height=5,warning=FALSE,message=FALSE,fig.cap='Partial regression plots for example regions that show omnibus level fwe significant differences from controls.'}
###################
###################
t1qc="T1wQC1+T1wQC2+T1wQC3+T1wQC4+"; t1qc='1+'
dtqc="+DTIQC1+DTIQC2+DTIQC3+DTIQC4"; dtqc='1+'
rsqc="+rsfQC1+rsfQC2+rsfQC3+rsfQC4"; rsqc='1+'
mydogU=mydog[ !is.na(mydog$TDX), ]
...
print( paste("Before matching: ",rsq::rsq(glm( studyNameNum ~ age_BL + 
commonSex + moca, data=mydogU)) ))
t.test( mydogU$age_BL[mydogU$studyName=='ADNI'], 
    mydogU$age_BL[mydogU$studyName=='PPMI'] )
mydogUa=mydogU[ mydogU$studyName == 'ADNI' & mydogU$age_BL <= 80.0, ]
mydogUp=mydogU[ mydogU$studyName == 'PPMI', ]
# temp=find_closest_subjects( mydogUp, mydogUa, 1, 'commonSex', 'age_BL')
mydogUa=match_data_frames( mydogUp, mydogUa, c("age_BL",'commonSex','moca'))
mydogU=rbind(mydogUa,mydogUp)

cogs = getNamesFromDataframe( c("updrs","score"), mydogU, exclusions=c(4,'_BL',"_delta") )
col2sbl = unique(c( nmstotest2, cogs, 'moca'  ))
mydogU = fillBaselineColumn( mydogU,
        col2sbl,
        'subjectID', 'yearsbl', 0, 
        fast=T, verbose=F )[[1]]
mydogU$hy_BL = factor( mydogU$hy_BL )
mydogU$yearsblr = round( mydogU$yearsbl )

t.test( mydogU$age_BL[mydogU$studyName=='ADNI'], 
    mydogU$age_BL[mydogU$studyName=='PPMI'] )
summary(glm( studyNameNum ~ age_BL * commonSex, data=mydogU))
print( paste("After matching: ",rsq::rsq(glm( studyNameNum ~ age_BL + commonSex+ moca, data=mydogU)) ))
dxform="  (1|commonID) + studyName*mrimfg + agebl1 + agebl2 + commonSex  + yearsbl "
if ( any(grepl("adjusted",nmstotest2)) ) {
  dxform="  (1|commonID) + agebl1 + agebl2 + commonSex  + yearsbl "
}
##############
dxglist=list()
dxdfT1=data.frame()
...
for ( voi in nmstotest2 ) {
  ...
  othdxform=dxform
  selector=!is.na( mydogU[,voi]) & 
#    ( ( mydogU$age_BL >= 45.0 & mydogU$studyName == 'PPMI' ) |
#    ( mydogU$age_BL <= 70.0 & mydogU$studyName == 'ADNI' ) ) &
    mydogU$imaging_protocol %in% c("1","2","3") & 
    mydogU$yearsbl <= maxy &
    mydogU$TDX %in% c( "CN", "PDSp-","PDSp+", "MCI-","PDLRRK2-","PDLRRK2+"  )
#    mydogU$TDX %in% c( "CN", "PDLRRK2-","PDLRRK2+", "MCI-", "MCI+" )
# temp=mydogU[selector,]
#    mydogU$TDX %in% c("PDLRRK2+","PDLRRK2-")
#    mydogU$TDX %in% c( "CN", "PDSp+","PASp-","PASp+","PDSp-", "MCI-", "MCI+")
  if ( antspymm_vartype(voi) == "T1" ) {
    selector = selector & subtyper::fs( mydogU$T1Hier_resnetGrade >= rgT1 )
#    othdxform=gsub("studyName","1",dxform)
    othdxform=paste0(t1qc,dxform)
  }
  ...
  if ( antspymm_vartype(voi) == "rsfMRI" ) {
    ...
    othdxform=paste0( rsqc, motname,"+rsfMRI_fcnxpro",fnum,"_minutes_original_data+rsfMRI_fcnxpro",fnum,"_minutes_censored_data+",dxform)
    selector = selector & subtyper::fs( mydogU$rsf1_ol_loop  <= 0.5 &
      mydogU$rsfMRI_fcnxpro129_FD_mean <= 0.5 )
  }
  if ( antspymm_vartype(voi) == "DTI" ) {
    othdxform=paste0(dtqc,motnamedt,"+",dxform)
    # othdxform=paste0(motnamedt,"+",dxform)
    selector = selector & subtyper::fs( mydogU$DTI1_ol_loop  <= 0.5 &
      mydogU$DTI_dti_FD_mean <= 5 )
  }
  longids = mydogU$subjectID[  mydogU$yearsbl >= 0.8 ]
  baseids = mydogU$subjectID[  mydogU$yearsbl == 0 ]
  bothids = intersect( longids, baseids )
  selectorlong = selector & mydogU$subjectID %in% bothids
  bform = paste( voi, " ~ brainVolume_BL+commonEdu+", othdxform )
  temp2 = mydogU[ selector, ]
  temp2L = mydogU[ selectorlong, ]
  age2df=data.frame(stats::poly(temp2$age_BL,2))
  age2dfL=data.frame(stats::poly(temp2L$age_BL,2))
  colnames(age2df)=c("agebl1",'agebl2')
  colnames(age2dfL)=c("agebl1",'agebl2')
  temp2=cbind( temp2, age2df )
  temp2L=cbind( temp2L, age2dfL )
  mylen = length( table( temp2$studyName ) )
  if ( mylen == 1 ) {
    bform=gsub("studyName","1",bform,fixed=TRUE)
  }
  newform = paste( bform, " * ", dxvar  )
#  temp2 = mydogU[ !is.na( mydogU[,voi]) &  mydogU$studyName %in% c('PPMI'), ]
  temp2 = scale_variables_in_equation( temp2, newform, variables_to_exclude=c(voi,'yearsbl'))
  temp2L = scale_variables_in_equation( temp2L, newform, variables_to_exclude=c(voi,'yearsbl'))
  for ( xxx in all.vars( as.formula(newform) )) {
    if ( is.factor( temp2[,xxx])) {
      contrasts(temp2[,xxx]) <- contr.sum
      contrasts(temp2L[,xxx]) <- contr.sum
      }
    }
#  temp2[,voi]=psych::winsor(temp2[,voi],tr=0.01)
  mdl=lmer( newform, data=temp2 )
  mdlL=lmer( newform, data=temp2L )
  mycoffs = coefficients( summary( mdl ) )
  mycoffsL = coefficients( summary( mdlL ) )
  myss = data.frame(tidy(interactions::sim_slopes(mdlL,'yearsbl','TDX',
    mod2.values=c(2.0))))
  rownames(myss)=myss[,'modx.value']
  anvpMX = min( mycoffs[ grep("PD",rownames(mycoffs)) ,5])
  anvpSS = min( myss[ grep("PD",myss[,'modx.value']) ,'p.value'])
  myem = data.frame(emmeans(mdl, ~ TDX | yearsbl,  at = list(yearsbl = c(0, 1, 2 ))))
  rownames(myem)=paste0(as.character(myem$TDX),"_",as.character(myem$yearsbl))
  # suppressMessages( anova(bmdl,mdl)$Pr[2] )
  dxdfT1[n,'voiraw']=voi
  dxdfT1[n,'voi']=voi
  dxdfT1[n,'n.base']=length(unique(temp2$commonID))
  dxdfT1[n,'n.PD-']=length(unique(temp2$commonID[temp2$TDX=='PDSp-' ]))
  dxdfT1[n,'n.PD+']=length(unique(temp2$commonID[temp2$TDX=='PDSp+' ]))
  dxdfT1[n,'n.long']=length(unique(temp2L$commonID))
  dxdfT1[n,'n.long.PD-']=length(unique(temp2L$commonID[temp2L$TDX=='PDSp-' ]))
  dxdfT1[n,'n.long.PD+']=length(unique(temp2L$commonID[temp2L$TDX=='PDSp+' ]))
#  dxdfT1[n,'n.long']=poptbl[2]
  dxdfT1[n,'minpMX']=anvpMX
  dxdfT1[n,'minpSS']=anvpSS
  dxdfT1[n,'CN.mean']=myem['CN_0','emmean']
  dxdfT1[n,'CN.mean+']=myem['CN_0','upper.CL']
  dxdfT1[n,'CN.mean-']=myem['CN_0','lower.CL']
  dxdfT1[n,'CN.change']=myss['CN','estimate']
  dxdfT1[n,'CN.change+']=myss['CN','conf.high']
  dxdfT1[n,'CN.change-']=myss['CN','conf.low']
  dxdfT1[n,'MCI-.mean']=myem['MCI-_0','emmean']
  dxdfT1[n,'MCI-.mean+']=myem['MCI-_0','upper.CL']
  dxdfT1[n,'MCI-.mean-']=myem['MCI-_0','lower.CL']
  dxdfT1[n,'MCI-.change']=myss['MCI-','estimate']
  dxdfT1[n,'MCI-.change+']=myss['MCI-','conf.high']
  dxdfT1[n,'MCI-.change-']=myss['MCI-','conf.low']
  dxdfT1[n,'PDSp-.mean']=myem['PDSp-_0','emmean']
  dxdfT1[n,'PDSp-.mean+']=myem['PDSp-_0','upper.CL']
  dxdfT1[n,'PDSp-.mean-']=myem['PDSp-_0','lower.CL']
  dxdfT1[n,'PDSp-.change']=myss['PDSp-','estimate']
  dxdfT1[n,'PDSp-.change+']=myss['PDSp-','conf.high']
  dxdfT1[n,'PDSp-.change-']=myss['PDSp-','conf.low']
  dxdfT1[n,'PDSp+.mean']=myem['PDSp+_0','emmean']
  dxdfT1[n,'PDSp+.mean+']=myem['PDSp+_0','upper.CL']
  dxdfT1[n,'PDSp+.mean-']=myem['PDSp+_0','lower.CL']
  dxdfT1[n,'PDSp+.change']=myss['PDSp+','estimate']
  ...
  chnd2 = data.frame(effectsize::epsilon_squared(mdl))
  rownames(chnd2)=chnd2[,'Parameter']
  othercolsnm2=c("e2.b","e2.y")
  dxdfT1[n,othercolsnm2]=chnd2[grep('TDX',rownames(chnd2)),'Epsilon2_partial']
  ...
  # 
  if ( n == 1 ) {
    unqdx=levels(temp2$DXSubAsyn)
    dxcts=data.frame(dx=unqdx,n=NA)
    rownames(dxcts)=unqdx
    for ( zzz in unqdx ) {
      dxsel = temp2$DXSubAsyn == zzz
...
#  if ( any(  mycoffs[ multigrep( c("PD","MCI"),rownames(mycoffs)), 5 ] < 1e-3 ) ) {
  ...
  if ( all( c(anvpSS,anvpMX) <= 0.05/length(nmstotest2) )  ) {
...
#    print(  knitr::kable( data.frame(mycoffs[-1,coffcols]), "latex", booktabs = TRUE,       caption=shorten_pymm_names(voi)) %>%      kableExtra::kable_styling(latex_options = c("striped", "scale_down")) )
    anvp1 = insight::format_p( myss[1,'p.value'], digits=4 )
    anvp2 = insight::format_p( myss[2,'p.value'], digits=4 )
    ttl=( paste( shorten_pymm_names(voi), "PD+ : ", anvp1, "PD- : ", anvp2  ) )
#    mygg = plot( predictorEffects(mdl,'TDX*yearsbl')[[2]], main=ttl,ylab=shorten_pymm_names(voi) )
#    dxglist[[length(dxglist)+1]]=mygg 
    manualColors = pal_futurama(palette = c("planetexpress"), alpha = 1)(4)
    manualColors = pal_jco( alpha = 1)(length(table(temp2$TDX)))
    pd <- ggplot2::position_dodge( 0.05 ) # move them .05 to the left and right
    ww = 0.5
    print( ggplot( myem,
      aes(x=yearsbl, y=emmean,
        group = TDX, color = TDX  )) + #, shape = Quant
      geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), 
      colour="black", width=ww, position=pd) +
      geom_line(lwd = 1, show.legend = FALSE ) +
      geom_point(position=pd, size=3, shape=21, fill="white") + # 21 is filled circle
      xlab( 'years' ) + ylab(paste(shorten_pymm_names(voi)))+
      theme(strip.text = element_text(size = rel(1.1))) +
      theme(legend.position = 'top') +
      guides(fill=guide_legend(title.position="top")) +
        theme_bw() + # scale_colour_manual( manualColors) +
        theme(text = element_text(size=20),
            axis.text.x = element_text(angle=0, vjust=1),
            legend.direction = "horizontal", legend.position = "top" )+ scale_colour_manual(values = manualColors ) )

#    print( interactions::probe_interaction(mdl, pred = 'yearsbl', modx = 'TDX',confint=F, facet.modx=F, interval=F, main.title=ttl, y.label=shorten_pymm_names(voi), data=temp2, colors=manualColors )$interactplot   )
#    print( dxglist[[length(dxglist)]] )
#    print( mygg )
#    visreg::visreg(mdl,'yearsbl',by='TDX')
    temp2L$yearsblr=round(temp2L$yearsbl)
#    print( plotSubtypeChange(temp2L,'commonID',vois[2],'TDX','yearsblr',whiskervar='se', ylab=shorten_pymm_names(voi)) ) + scale_colour_manual(values = manualColors )
#    ( visreg::visreg( mdl, 'yearsbl' , 'TDX', main=ttl))
#    Sys.sleep(1)
    print( mycoffs[-c(1:3),coffcols] )
    print("simple slopes")
    print(myss[, c("statistic","p.value","modx.value", "estimate", "conf.low","conf.high")])
    print(paste(shorten_pymm_names(voi),insight::format_p(anvpMX),insight::format_p(anvpSS)))
    print( dxdfT1[n,othercolsnm2] )
#    print( interpret_simlr_vector(mysim$simlrX, mysim$mats,  gsub("_adjusted","",voi) ) )
...
##########################################################
dxdfT1=dxdfT1[ !dxdfT1$singular,]
psel = subtyper::fs( 
  p.adjust( dxdfT1$minpMX, mycorr ) <= .05 &
  p.adjust( dxdfT1$minpSS, mycorr ) <= .05 )
##########################################################
if ( TRUE ) {
...
if ( length(dxglist) > 0 )
  grid.arrange( grobs = dxglist, nrow=4, top='Group differences in IPDs from controls.' )
...
dxdfT1selY = dxdfT1sel[ order( apply( dxdfT1sel[,c("p.yearsbl.PDSp-","p.yearsbl.PDSp+")], FUN=min, MARGIN=1)), ]
dxdfT1selB = dxdfT1sel[ order( apply( dxdfT1sel[,c( "p.PDSp-","p.PDSp+")], FUN=min, MARGIN=1)), ]

prod1 = dxdfT1sel[,"p.yearsbl.PDSp-"] * dxdfT1sel[,"p.yearsbl.PDSp+"]
dxdfT1selY = dxdfT1sel[ order( prod1 ), ]
prod1 = dxdfT1sel[,"p.PDSp-"] * dxdfT1sel[,"p.PDSp+"]
dxdfT1selB = dxdfT1sel[ order( prod1 ), ]

dxdfT1selcols = colnames(dxdfT1sel)[ !(colnames(dxdfT1sel) %in% c("res","singular","voiraw",'minpMX',"minpSS", getNamesFromDataframe("CN",dxdfT1sel), getNamesFromDataframe("LRRK",dxdfT1sel), getNamesFromDataframe("mean",dxdfT1sel), getNamesFromDataframe("change",dxdfT1sel)))]
...
# Print the ztable object, ensuring it's treated as LaTeX code
dxdfT1selBz=dxdfT1selB[, dxdfT1selcolsB]
dxdfT1selYz=dxdfT1selY[, dxdfT1selcolsY]
colnames(dxdfT1selYz)=gsub("yearsbl","y",colnames(dxdfT1selYz))
if ( "d.y.1" %in% colnames(dxdfT1selYz) ) {
  dxdfT1selYz=dxdfT1selYz[,!(colnames(dxdfT1selYz) %in% "d.y.1")]
}
# baseline t1 top 20
topk=20
gg=grep("t1",dxdfT1selBz$voi)
dxdfT1selBzT1=head( dxdfT1selBz[ gg, ], topk )
ggdt=grep("dti",dxdfT1selBz$voi)
dxdfT1selBzDT=head( dxdfT1selBz[ ggdt, ], topk )
ggrs=grep("rsf",dxdfT1selBz$voi)
dxdfT1selBzRS=head( dxdfT1selBz[ ggrs, ], topk )

gg=grep("t1",dxdfT1selYz$voi)
dxdfT1selYzT1=head( dxdfT1selYz[ gg, ], topk )
ggdt=grep("dti",dxdfT1selYz$voi)
dxdfT1selYzDT=head( dxdfT1selYz[ ggdt, ], topk )
ggrs=grep("rsf",dxdfT1selYz$voi)
dxdfT1selYzRS=head( dxdfT1selYz[ ggrs, ], topk )

dxdfT1selz=ztable( rbind(dxdfT1selBzT1,dxdfT1selBzDT,dxdfT1selBzRS), zebra=2)
mycap='Significant (fwe based on omnibus model p-value) diagnostic group i.e. time independent effects in PPMI PD.  Empty cells represent small effects.'
mycap='Table 5. Time independent group effects in PPMI PD. Top 20 per modality.'
...
dxdfT1selz=ztable(rbind(dxdfT1selYzT1,dxdfT1selYzDT,dxdfT1selYzRS),zebra=2)
mycap='Significant (fwe based on omnibus model p-value) longitudinal group effects in PPMI PD. Empty cells represent small effects.'
mycap='Table 6. Longitudinal group effects in PPMI PD. Top 20 per modality.'
print(dxdfT1selz, type = "latex",size=2,caption=mycap,sidewaystable=TRUE)
```

```{r quicklmcog,echo=FALSE,eval=!exists("mydogp2"),fig.width=10,fig.height=6,warning=FALSE,message=FALSE,fig.cap='Partial regression interaction plots show a representative region for each of the four UPDRS outcomes that we test.  Each panel demonstrates how different levels of the IDP (x-axis) lead to different amounts of change over time. For example, the slope of the relationship btween MD asymmetry in the internal capsule and change in UPDRS2 score increases over time with the largest differences occuring at the right side of the top right panel.  This indicates that subjects with higher levels of baseline MD asymmetry in this IDP progress more rapidly on this score.'}
#########################################################
...
mydogp2[ is.na(mydogp2$LEDD),'LEDD']=0
mydogp2[ is.na(mydogp2$duration_yrs),'duration_yrs']=0
# mydogp2[ mydogp2$studyName =='ADNI','TDX'] = mydogp2[ mydogp2$studyName =='ADNI','joinedDX']
cogtotest="updrs_totscore"
cogtotest='updrs3_score' #  "updrs_totscore"
# cogtotest='updrs2_score' #  "updrs_totscore"
cogtotest=c('updrs1_score','updrs3_score','updrs2_score','moca') #  "updrs_totscore"
stadfrsf=data.frame()
cogglist=list()
signames = sample(nmstotest2)
for ( voi in signames ) {
  voi = paste0(voi,"_BL")
  for ( cog in cogtotest ) {
    n = nrow(stadfrsf)+1
    maxy=2.5
    if ( cog != 'moca') {
      selector = !is.na( mydogp2[,voi]) & (mydogp2$TDX %in% c("PDSp+")) & 
        mydogp2$yearsbl <= maxy & subtyper::fs( mydogp2$T1Hier_resnetGrade >= rgT1 )
    } else {
      selector = !is.na( mydogp2[,voi]) &(mydogp2$TDX %in% c("PDSp+")) & 
        mydogp2$yearsbl <= maxy & subtyper::fs( mydogp2$T1Hier_resnetGrade >= rgT1 )
    }
    if ( antspymm_vartype( voi ) == "T1" ) {
      motnameu=t1qc
      selector = selector & subtyper::fs( mydogp2$T1Hier_resnetGrade >= rgT1 )
    }
    if ( antspymm_vartype( voi ) == "rsfMRI" )  {
      motnameu = paste( motname, rsqc )
      selector = selector & subtyper::fs( mydogp2$rsf1_ol_loop  <= 0.5 &
        mydogp2$rsfMRI_fcnxpro129_FD_mean <= 0.5 )
      }
    if ( antspymm_vartype( voi ) == "DTI"   )  {
      motnameu = paste( motnamedt,  dtqc )
      selector = selector & subtyper::fs( mydogp2$DTI1_ol_loop  <= 0.5 &
        mydogp2$DTI_dti_FD_mean <= 5 )
    }
    if ( cog != 'moca') {
      cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+(1|commonID) + duration_yrs + LEDD + commonEdu + age_BL + commonSex +  ( yearsbl )" )
    } else cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+(1|commonID) + commonEdu + age_BL + commonSex +  ( yearsbl )" )
    longids = mydogp2$subjectID[  mydogp2$roundyears >= 1 & selector ]
    baseids = mydogp2$subjectID[  mydogp2$roundyears == 0 & selector ]
    bothids = intersect( longids, baseids )
    temp2 = mydogp2[ selector & mydogp2$subjectID %in% bothids, ]
    # temp2 = mydogp2[ selector, ]
...
#    print(voi)
#    print( table( temp2$TDX ) )
    newform = paste( cogformcommon, " * TDX * ", voix )
    newform = paste( cogformcommon, " * ", voix )
    if ( nrow(temp2) > 10 ) {
      temp2 = scale_variables_in_equation( temp2, newform , 
        variables_to_exclude=c('yearsbl',voix))
#      mdl=suppressMessages(lmer( newform, data=temp2 ))
      voixq=paste0(voix,"_q")
      myst = trainSubtypeUni( temp2, voix, c("L","M","H"), subtypename=voixq )
      temp2 = predictSubtypeUni( temp2, myst, 'commonID', 'yearsbl', 0 )
      newform = as.formula( paste( cogformcommon, " * ", voixq ) )
      for ( xxx in all.vars( newform )) {
        if ( is.factor( temp2[,xxx]))
          contrasts(temp2[,xxx]) <- contr.sum
      }
  ...
  #    stadfrsf[n,'n.long']=poptbl[2]
      ...
      if (  anvp <= 1e-4 ) {
        chnd = data.frame(effectsize::epsilon_squared(mdl))
        rownames(chnd)=chnd[,'Parameter']
        othercolsnm=c("e2.b","e2.y")
        stadfrsf[n,othercolsnm]=chnd[grep(voix,rownames(chnd)),'Epsilon2_partial']
...
#        gg2 =interactions::probe_interaction(mdl, modx = 'yearsbl', 
 #         pred = !!sym(voix) ,confint=F, interval = F)$interactplot 

  #      mygg = plot( predictorEffects(mdl,paste0(voix,'*yearsbl'))[[2]], main=ttl,ylab=shorten_pymm_names(voix) )
#        if ( length( othercols ) == 2 ) {
#          mygg = plot( predictorEffects(mdl,paste0('yearsbl*',voix))[[2]], main=ttl,ylab=cog )
#
#        } else mygg = plot( predictorEffects(mdl,paste0(voix)), main=ttl,ylab=cog )

#        cogglist[[length(cogglist)+1]]=mygg

      emmform = formula( paste("~", voixq,"| yearsbl") )
      myem0 = emmeans(mdl, emmform,  at = list(yearsbl = c(0, 1, 2 )))
      myem = data.frame( myem0 )
      rownames(myem)=paste0(myem[,voixq],"_",as.character(myem$yearsbl))
      pd <- ggplot2::position_dodge( 0.05 ) # move them .05 to the left and right
      ww = 0.5
      print( ggplot( myem,
        aes(x=yearsbl, y=emmean, group=!!sym(voixq), color=!!sym(voixq) )) +
        geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), 
        colour="black", width=ww, position=pd) +
        geom_line(lwd = 1, show.legend = FALSE ) +
        geom_point(position=pd, size=3, shape=21, fill="white") + # 21 is filled circle
        xlab( 'years' ) + ylab(paste("change:",cog))+
        theme(strip.text = element_text(size = rel(1.1))) +
        theme(legend.position = 'top') +
        guides(fill=guide_legend(title.position="top")) +
          theme_bw() + # scale_colour_manual( manualColors) +
          theme(text = element_text(size=20),
              axis.text.x = element_text(angle=0, vjust=1),
              legend.direction = "horizontal", legend.position = "top" )+ scale_colour_manual(values = manualColors ) )
        print( mycoffs[-1,c(4:5)])
        print( cog )
        print( stadfrsf[n,] )
  #      gg2 =interactions::probe_interaction(mdl, pred = 'yearsbl', modx = !!sym(voix) ,confint=F, interval = F)$interactplot
...
#####################
stadfrsf = stadfrsf[!stadfrsf$singular,]
stadfrsf2 = stadfrsf[ stadfrsf$cog != 'moca', ]
sigth=0.05
# for ( cog in cogs ) 
{
  mypvs = stadfrsf2$anv
#  mypvs[ stadfrsf$cog != cog  ] = NA
  ...
  stadfrsf2[psel2,'sig']='fdr'
  stadfrsf2[psel,'sig']='fwe'
  if ( sum(psel ) > 0 )
    temp=stadfrsf2[psel | psel2,]
    qdfsig=temp
#    qdf = rbind( qdf, temp )
  }
if ( !is.null( qdfsig ) ) {
  ...
  qdfcols =  c("cog", "voi" , 'n.base', "anv", 'sig', othercolsnm)
  qdfsig = qdfsig[,qdfcols]
  qdfsig$voi=gsub("_",".",qdfsig$voi,fixed=TRUE)
  qdfsig$cog=gsub("_",".",qdfsig$cog,fixed=TRUE)
  qdfsig = na.omit( qdfsig[!duplicated(qdfsig),] )
}
###########################################################
if ( length( cogglist ) > 0 )
  grid.arrange( grobs = cogglist, nrow=2, top='Interactions between baseline IDPs and UPDRS changes within PD.' )
###########################################################################
```


```{r updrsresults,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE,results='asis'}
# Print the ztable object, ensuring it's treated as LaTeX code
qdfsigz=ztable(na.omit(qdfsig[subtyper::fs(qdfsig$sig == 'fwe'),]),zebra=2,tabular=F)
qdfsigz=ztable(na.omit(qdfsig[,]),zebra=2,tabular=F)
mycap='Table 7. Significant (fwe) IDP to MDS-UPDRS longitudinal effects for PD+: T1w, DTI and rsfMRI'
print(qdfsigz, type = "latex",size=3,caption=mycap,sidewaystable=FALSE)
```



## Baseline IDP to longitudinal UPDRS effects in structural, white matter and resting functional measurements
...
The outcome is $\text{UPDRSX}_{delta}$ indicating change in a given UPDRS score.  The majority of these variables are as defined previously.  However, we introduce control for treatment effects (LEDD) as well as baseline values of the given score.  This latter variable approximates a control for modality-specific disease severity.  As such, these models are relatively conservative in terms of their attribution of variance to baseline IDP values (i.e. $IDP_\text{bl}$).  The predictor of interest, here, is $years_\text{bl}  *  IDP_\text{bl}$ which allows assessment of the impact of baseline brain health on the change in the given UPDRS score.  These models are assessed in the range of baseline to 2.5 years change.  The effects are visualized through partial residual plots for select IDPs of interest as in Figure 9.  Effect sizes and $p$-values for each significant (fwe) pair of outcomes and IDPs in in Table 7.  Overall, these models demonstrate that baseline brain state -- as measured by each modality of interest -- may contribute to acceleration/deceleration of changes in UPDRS scores.  However, we accentuate that these models are relatively simple and linear; as such, they yield only rough suggestions that additional modeling effort may be warranted to understand the differential value of these IDPs across the spectrum of PD symptomology.

### Multi-view example: M3RI predicts change in UPDRS1 in 2 to 4 year followup

Here, we dissect the influence of M3RI neuroimaging biomarkers on the progression of Unified Parkinson's Disease Rating Scale (UPDRS) Part I scores [@chahine_predicting_2019]. The analysis focuses on three neuroimaging biomarkers indicative of structural and functional brain integrity: asymmetry in the putamen volume as derived from T1-weighted images, motor cortex connectivity metrics from resting-state fMRI, and mean diffusivity measures from DTI in the superior longitudinal fasciculus. We construct three linear regression models that predict change in UPDRS1 from baseline data.  The base model uses only clinical information.  We build two additional IDP models: the first includes only the putamen IDP; the second includes IDPs from each modality. This last model extended the clinical reference model by incorporating all three neuroimaging biomarkers simultaneously, allowing for the assessment of their improvement in predicting motor symptom progression in the 2 to 4 year range of the followup data. The models include covariates such as LEDD, disease duration, baseline motor function, and demographic information.


```{r multiview,echo=FALSE,eval=FALSE,fig.width=10,fig.height=5,message=FALSE,warning=FALSE,fig.cap="Resampled estimates of multi-view regression for predicting change in UPDRS1 scores based on three different modalities demonstrate improved expected peformance.  Vertical dashed lines show median performance.",cache=TRUE}
# this is so-so silly-ish
library(groupdata2)
library(xpectr)
cog='updrs_totscore'
cog='updrs1_score'
maxy=3000.0
vois = c(
  "DTI_mean_md.fornix_.cres..stria_terminalis_.can_not_be_resolved_with_current_resolution..Asym.jhu_icbm_labels_1mm_adjusted_BL",
  "rsfMRI_fcnxpro129_BasalGanglia_2_VisPeri_adjusted_BL",
  "T1w_mean_Asym_posterior_cingulate_adjusted_BL"
)
#  "DTI_mean_md.superior_longitudinal_fasciculus.LRAVG.jhu_icbm_labels_1mm" )
...
cogv=paste0(cog,c("_BL","_delta"))
raneffstr=''
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs + DTI_dti_FD_max_BL+ rsfMRI_fcnxpro129_FD_max_BL  + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl ) + ( yearsbl )  ")
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs +  LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl )   ")
uvform =paste( bform, "   *  ( ", voisx[1] , ")" )
mvform =paste( bform, " + ( yearsbl )  *  ( ", paste(voisx,collapse="+") , ")" )
temp2 = mydogp[ !is.na( mydogp[,vois[3]]) & !is.na( mydogp[,vois[2]]) &
#  mydogp$yearsbl <= maxy, ]
  mydogp$TDX == "PDSp+" & mydogp$yearsbl <= maxy & 
  mydogp$T1Hier_resnetGrade_BL > 1.02, ]
#  mydogp$TDX != "CN" & mydogp$yearsbl <= maxy, ]
for ( x in 1:length(vois) ) {
  temp2[,voisx[x]]=temp2[,vois[x]]
}
# temp2[,paste0(cog,"_delta")]=psych::winsor( temp2[,paste0(cog,"_delta")], 0.02 )
...
for ( jj in 1:n ) {
  ...
  bmdl=suppressMessages(lm( bform, data=train_test[[1]] ))
  mdlu=suppressMessages(lm( uvform, data=train_test[[1]] ))
  mdl=suppressMessages(lm( mvform, data=train_test[[1]] ))
  highy=train_test[[2]]$yearsbl >= 0.80 & train_test[[2]]$yearsbl < 2.2
#  highy=train_test[[2]]$yearsbl >= 1.80 & train_test[[2]]$yearsbl < 5
...
#    print( range( train_test[[2]]$yearsbl ) )
  #  print( length( fixef(bmdl)))
    ...
    mypred = predict( mdl, newdata=train_test[[2]])#, allow.new.levels = TRUE )
...
if ( FALSE ) {
  ...
  g1=cvdf$model =='clin'
  g2=cvdf$model =='idp3'
  mvanvp = t.test(  cvdf[g1,m] ,   cvdf[g2,m] ,  paired=TRUE )$p.value
  g1=cvdf$model =='clin'
  g2=cvdf$model =='idp1'
  mvanvp = t.test(  cvdf[g1,m] ,   cvdf[g2,m] ,  paired=TRUE )$p.value
  g1=cvdf$model =='idp1'
  ...
  mvanvpu = t.test(  cvdf[g1,m] ,   cvdf[g2,m] ,  paired=TRUE )$p.value
  print( gghistogram(cvdf[cvdf$model %in% c("clin","idp3"),], 
    x = m, add = "mean", rug = TRUE,
    color = "model", fill = "model", palette='jco', xlab='MAE: real to predicted change' ) )
}
####################################################
...
The effects of neuroimaging biomarkers on UPDRS Part I score changes are predictive both independently and even more so in combination. This can be quantified in several ways;  here, we use a resampling approach to generate many runs of this modal comparison in different subsets of the data.  Each independent train-test run uses `r 100*mypartrate`\% of subjects in training with a minimum of 6 left out subjects in each test set.  Aggregating across many simulations provides the opportunity to estimate the expected additional value of the 3 IDP vs 1 IDP vs baseline clinical model.  The 1 IDP model is:
$$
\begin{split}
\text{UPDRS1}_{delta} \approx  \text{UPDRS1}_\text{bl} + duration_\text{yrs} \\
...
\end{split}
$$
The most prominent predictors in this model include interaction of diagnostic group with years from baseline, LEDD and the baseline score as well as the IDP. The 3 IDP model adds rsfMRI connectivity between motor and cognitive control regions and mean diffusion:
$$
\begin{split}
\text{UPDRS1}_{delta} \approx  \cdots + years_\text{bl}  * \\
 (  \cdots ~+~rsf.sommota.2.contc.bl~+~dti.mean.md.fornix.asym.bl )
\end{split}
$$
Paired $t$-test show the 3 IDP model improves correlation between real and predicted UPDRS1 change scores versus both the baseline (`r insight::format_p(mvanvp3,digits=4)`) and 1 IDP model (`r insight::format_p(mvanvpu,digits=4)`). Figure 10 demonstrates the distribution of performance between the clinical and 3 IDP models over all of the simulations.  This example demonstrates the potential of multimodal neuroimaging and integrative statistical approaches in PD and neurodegenerative disease research more generally.

In summary, this tabulated multi-modality MR IDP dataset for PPMI -- derived from deeply validated open source methods -- represents a valuable opportunity to help standardize as well as advance PD M3MRI research. It simplifies the analysis of complex imaging data, potentially accelerates the discovery of novel insights into PD progression and effects.  The timing of this data release is critical given the newly available SAA biomarker. Additionally, this methodology holds promise for broader applications, potentially benefiting research into other neurological conditions.
...
```{r simlrBL,echo=FALSE,eval=FALSE,fig.width=10,fig.height=5,message=FALSE,warning=FALSE,fig.cap="Resampled estimates of multi-view ML for predicting change in UPDRS scores based on three different modalities demonstrate improved expected peformance.  Vertical dashed lines show median performance.",cache=TRUE}
maxy=4.5
voisx=sample(nmstotest2,20) # simnames
cogv = paste0( "updrs1_score", c("_BL","_delta"))
bform =paste( cogv[2], " ~ ", cogv[1], "  +  LEDD + age_BL + commonSex + brainVolume_BL + ( yearsbl )   ")
uvform =paste( bform, "   *  ( ", voisx[1] , ")" )
mvform =paste( bform, " + ( yearsbl )  *  ( ", paste(voisx,collapse="+") , ")" )
cogs = getNamesFromDataframe( c("updrs","score"), ppmitrim, exclusions=c(4,'_BL',"_delta") )
temp2 = fillBaselineColumn( ppmitrim,
        c( cogs, 'moca' ),
        'subjectID', 'yearsbl', 0, 
        fast=T, verbose=F )[[1]]

temp2 = temp2[ !is.na( temp2[,cogv[2]] ) &
    !is.na( temp2[,voist1[1]]) & 
    !is.na( temp2[,voisdt[1]]) &
    !is.na( temp2[,voisrs[1]]) &
  subtyper::fs(temp2$DXSubAsyn == "PDSporadicPositive" & temp2$yearsbl <= maxy & 
  temp2$T1Hier_resnetGrade_BL > 1.02), ]
...
#    print( range( train_test[[2]]$yearsbl ) )
  #  print( length( fixef(bmdl)))
    mypredb = predict( bmdl, newdata=train_test[[2]])#, allow.new.levels = TRUE )
    mypredu = predict( mdlu, newdata=train_test[[2]])#, allow.new.levels = TRUE )
    mypred = predict( mdl, newdata=train_test[[2]][,all.vars(as.formula(mvform))])#, allow.new.levels = TRUE )
  #  plot( train_test[[2]][,paste0(cog,"_delta")], mypred )
    jjj=nrow(cvdf)+1
    cvdf[jjj,'nte']=length(unique(train_test[[2]]$commonID))
    cvdf[jjj,'ntr']=length(unique(train_test[[1]]$commonID))
