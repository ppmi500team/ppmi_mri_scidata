...
mysim = antspymm_simlr( ppmitrim, ppmitrim$yearsbl==0, 
    energy='cca', nsimlr=8, covariates=NULL, myseed=808, doAsym=2, 
    returnidps=FALSE,
    exclusions=c("_delta","_BL",'_adjusted', '122','134' ), 
    # sparseness=0.95,
    # inclusions=',
    verbose=TRUE)
...
nmstotest2 = c( voist1, voisdt, voisrs )
...
dxdfT1sel = dxdfT1sel[ order( dxdfT1sel$minpSS ), ]
...
ggdt=grep("dt",dxdfT1selBz$voi)
dxdfT1selBzDT=head( dxdfT1selBz[ ggdt, ], topk )
ggrs=grep("rs",dxdfT1selBz$voi)
...
for ( voi in signames ) {
  ...
  for ( cog in cogtotest  ) {
    ...
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+(1|commonID)+ imaging_protocol + duration_yrs + LEDD + commonEdu + age_BL + commonSex + ( yearsbl )" )
...
maxy=2.5
voist1 = c( 
    getNamesFromDataframe(  "t1PC", mysim$demog, exclusions=c("_BL","_delta","_adjusted") ),
    getNamesFromDataframe(  "t1aPC", mysim$demog, exclusions=c("_BL","_delta","_adjusted") ) )
voisrs = getNamesFromDataframe(  "rsPC", mysim$demog, exclusions=c("_BL","_delta","_adjusted") )
voisdt = c( 
    getNamesFromDataframe(  "dtPC", mysim$demog , exclusions=c("_BL","_delta","_adjusted")),
    getNamesFromDataframe(  "dtaPC", mysim$demog, exclusions=c("_BL","_delta","_adjusted") ) )
