...
col2sbl = unique(c( nmstotest2, cogs ))
...
#    ( mydogU$age_BL <= 70.0 & mydogU$studyName == 'ADNI' ) ) &
    ...
    mydogU$yearsbl <= maxy &
    mydogU$TDX %in% c( "CN", "PDSp-","PDSp+", "MCI-"  )
...
#    dxglist[[length(dxglist)+1]]=mygg 
    ...
    manualColors = pal_jco( alpha = 1)(4)
...
dxdfT1selcols = colnames(dxdfT1sel)[ !(colnames(dxdfT1sel) %in% c("res","singular","voiraw",'minpMX',"minpSS", getNamesFromDataframe("CN",dxdfT1sel), getNamesFromDataframe("mean",dxdfT1sel), getNamesFromDataframe("change",dxdfT1sel)))]
...
cogtotest=c('updrs1_score','updrs3_score','updrs2_score') #  "updrs_totscore"
...
for ( voi in signames ) {
  ...
  for ( cog in cogtotest ) {
    ...
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+(1|commonID) + duration_yrs + LEDD + commonEdu + age_BL + commonSex + TDX * yearsbl + ( yearsbl )" )
...
mycap='Table 7. Significant (fwe) IDP-UPDRS longitudinal effects for PD+: T1w, DTI and rsfMRI'
