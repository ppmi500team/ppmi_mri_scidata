...
```{r quicklmdx,echo=FALSE,eval=buildsci,fig.width=9,fig.height=7,warning=FALSE,message=FALSE}
vartype <- function( x ) {
  if ( length(grep("T1",x) > 0 ) ) return( "T1" )
  if ( length(grep("rsfMRI",x) > 0 ) ) return( "rsfMRI" )
  if ( length(grep("DTI",x) > 0 ) ) return( "DTI" )
  if ( length(grep("NM2",x) > 0 ) ) return( "NM2DMT" )
  return( NA )
}
######################################################################
...
for ( voi in nmstotest ) {
  ...
  othdxform=dxform
  if ( vartype(voi) == "rsfMRI" ) othdxform=paste0(motname,"+",dxform)
  if ( vartype(voi) == "DTI" ) othdxform=paste0(motnamedt,"+",dxform)
...
#  if ( any(  mycoffs[ multigrep( c("PD","MCI"),rownames(mycoffs)), 5 ] < 1e-3 ) ) {
  if ( anvp < 1e-4 ) {
    ...
    ttl=( paste( shorten_pymm_names(voi), myrez, " + ", mydx,  " : ", anvp ) )
...
```{r quicklmcog,echo=FALSE,eval=buildsci,fig.width=9,fig.height=6,warning=FALSE,message=FALSE}
#########################################################
...
for ( cog in  cogs  ) {
  for ( voi in nmstotest ) {
    ...
    voi = paste0(voi,"_BL")
    maxy=5.0
    if ( vartype( voi ) == "T1" ) motname=""
    if ( vartype( voi ) == "rsfMRI" )  {
...
knitr::kable(qdf, "latex", booktabs = TRUE, caption='Significant IDP-UPDRS longitudinal  effects: T1w (n.s.), DTI and rsfMRI') %>%
  kableExtra::kable_styling(latex_options = c("striped", "scale_down"))
```
