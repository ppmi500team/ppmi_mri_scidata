...
joinedem$joinedDX[ 
  multigrep( c( "PDPRKN", "PDSNCA"), joinedem$joinedDX )]='GenPD'
joinedem$joinedDX[ 
...
mlevs = c( "CN", "GenAR", "SporadicAR","GenPD", 'LRRK2PD', 'GBAPD', "SporadicPD")
...
* DTI ENIGMA https://www.medrxiv.org/content/10.1101/2024.01.16.24301235v1.supplementary-material

* Diffusion basis spectrum imaging detects pathological alterations in substantia nigra and white matter tracts with early-stage Parkinson disease https://link.springer.com/article/10.1007/s00330-023-09780-0
...
myregionszRSFz=ztable(myregions2RSF[,c('IDP','connectivity')],tabular=TRUE,size=2,caption='PD IDPs in ANTsPyMM: DTI L/R average and asym for both fractional anisotropy (FA) and mean diffusion (MD) (not shown)')
myregionszDTIz=ztable(myregions2DTI[grep("dti.fa",myregions2DTI$IDP,fixed=TRUE),c('IDP','anat')],tabular=TRUE,size=2,caption='PD IDPs in ANTsPyMM: rsfMRI bilateral inter or intra-network connectivity  (Yan, et. al. homotopic parcellation nomenclature)')
print(myregionszT1z)
...
```{r quicklmcog,echo=FALSE,eval=buildsci2,fig.width=12,fig.height=4,warning=FALSE,message=FALSE}
#########################################################
...
for ( cog in  cogs  ) {
  for ( voi in nmstotest ) {
    ...
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+ (1|SITE)+ (1|commonID)  + duration_yrs + ",motname," + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl )*TDX + ( yearsbl )" )
...
As in the prior section, we employ LMMs to measure the relationship of baseline IDP values to clinical decline as evaluated by UPDRS 1, 2 and 3 (off and on) scores.  While these measurements have well-documented limitations in terms of reliability and interpretability [@FIXMEMILESTONESPAPER], they are consistently available in PPMI.  These exemplar assessments differ from the prior section in that they focus only on PD subjects.  These models are of the form:

$$
\begin{split}
\text{UPDRS}_{delta} \approx (1|ID) + (1|SITE) + \text{UPDRS}_{bl} + \text{BV}_\text{bl} + Edu + duration_\text{yrs} + \\
  modality\_specific\_covariates  + age_\text{bl} + \\ 
  Sex_\text{bio}  + years_\text{bl}   *  \text{DX} + years_\text{bl}  *  IDP_\text{bl}
\end{split}
$$

The majority of these variables are as defined previously.  However, we introduce control for treatment effects (LEDD) as well as baseline values of the given score.  This latter variable approximates a control for modality-specific disease severity.  As such, these models are relatively conservative in terms of their attribution of variance to baseline IDP values (i.e. $IDP_\text{bl}$).  The predictor of interest, here, is $ years_\text{bl}  *  IDP_\text{bl}$ which allows assessment of the impact of baseline brain health on the change in given UPDRS score.  These models are assessed in the range of baseline to 2.5 years change.  The effects of interest are visualized through partial residual plots for select IDPs of interest as in Figures FIXME.
...
The effects of neuroimaging biomarkers on UPDRS Part I score changes are predictive both independently and even more so in combination. This can be quantified in several ways;  here, we report the omnibus model 2 compared to model 1 where these are defind as (model 1, including only asymmetry in baseline putamen volume as in IDP):

$$
\begin{split}
updrs1_text{change} ~  updrs1_\text{bl} + (1|SITE)+ (1|commonID) + duration_\text{yrs} + DTI_\text{FD} + rsfMRI_\text{FD}  + LEDD + Edu + age_\text{bl} + Sex + \text{BV}_\text{bl} + years_\text{bl}~*~DX + years_\text{bl}  * t1.vol.bn.str.pu.asym.bl  + years_\text{bl}  *  (  t1.vol.bn.str.pu.asym.bl+rsf.sommota.2.contc.bl+dti.mean.md.ant.int.cap )
\end{split}
$$
