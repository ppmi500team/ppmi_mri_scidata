...
#    mygg1=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=F ) + ggtitle(paste("L:",voi)) + labs(y = NULL) + theme(legend.position = "top") )
    mygg2=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=T, plot.points=T ) + ggtitle(paste("L:",voix)) + labs(y = NULL) + theme(legend.position = "top") )
...
mycap='Table 5. Time independent group effects in PPMI PD. Top-k per modality.'
...
mycap='Table 6. Longitudinal group effects in PPMI PD. Top-k per modality.'
print(dxdfT1selz, type = "latex",size=2,caption=mycap,sidewaystable=TRUE)
```
...
if ( ! file.exists( paste0(simfile,'_t1_simlr.csv') ) ) {
    ...
    mycc = complete.cases( ppmitrim[,vars3]) & ( isav45pos | issaapos )
...
for ( voi in (1:npc) ) {
  for ( cog in sample(coggers) ) {
    ...
    covarsX=" commonEdu + LEDD + duration_yrs + age_BL + commonSex + ( yearsbl )"
    redspns=paste0(cog,"_delta")
    cogformcommon=paste( redspns, "~ ",paste0(cog,"_BL"),"+joinedDX+(1|commonID)+(1|SITE2)+ ", covarsX )
#    cogformcommon=paste( redspns, "~ ",paste0(cog,"_BL"),"+(1|commonID)+ ", covarsX )
    ...
    mydogpnna=mydogp[ complete.cases( mydogp[,all.vars(as.formula(newform))] ) & 
      ...
      mydogp$yearsbl <= maxy, ]
    topkval = round( 0.95 * nrow(mydogpnna) )
...
#      if ( paste0(cog,voi) %in% c( "updrs1_score91", "updrs3_score31","updrs2_score64") ) {
        ...
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=jtools::effect_plot( resultsX$full_model,  !!sym(vars3[k]), 
              interval=T, facet.modx=F, plot.points = T ) + ggtitle(paste("X:",vars3[k])) + theme(legend.position = "none") 
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=( interactions::interact_plot( mdl,  'yearsbl', !!sym(vars3[k]), 
              interval=T, facet.modx=F , plot.points = T ) + ggtitle(paste("L:",vars3[k]))  + theme(legend.position = "none") )
        mlist = lappend( 
          compare_model_predictions( resultsX$base_model, resultsX$full_model, mydogpnnaBase, cog ),
          compare_model_predictions( bmdl, mdl, mydogpnnaLong, redspns  )
        )
...
# Print the ztable object, ensuring it's treated as LaTeX code
toviz = na.omit(qdfsig[qdfsig$sig %in% c('fwe','fdr'), ])
...
mycap=paste0('Table 7. Significant (fdr) SiMLR IDP to clinical measurements: top-k for each score.')
