...
We supplement these PPMI data with subjects from ADNI due to the relative dearth of control longitudinal neuroimaging in PPMI.  As with PPMI, we attach ANTsPyMM IDPs to `ADNIMERGE_10Feb2024` using the `subtyper` function `merge_ADNI_antspymm_by_closest_date`.  Select subjects from ADNI are considered for merging with PPMI if they are not diagnosed wtih Alzheimer's disease or mild cognitive impairment (MCI) and have acceptable quality neuroimaging. The ADNI cohort is significantly older (mean of 72.3 years versus 63.9 in PPMI).
...
```{r quicklmdx,echo=FALSE,eval=!exists("mydogU"),fig.width=8,fig.height=4.5,warning=FALSE,message=FALSE,fig.cap='Partial regression plots for example regions that show omnibus level significant differences from controls.'}
###################
...
nms2viz = c("t1.vol.sncdp.", "t1.vol.bn.str.pu.asymdp.", "dti.fa.deep.snc", "rsf.p2.sommotb.2.striatum")
...
#    mygg1=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=F ) + ggtitle(paste("L:",voi)) + labs(y = NULL) + theme(legend.position = "top") )
    mygg2=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=T, plot.points=F ) + ggtitle(paste("L:",voix)) + labs(y = NULL) + theme(legend.position = "top") )
...
```{r quicklmdxsetdown,echo=FALSE,eval=TRUE,fig.width=12,fig.height=14,warning=FALSE,message=FALSE,fig.cap='Partial regression plots for example regions that show omnibus level fwe significant differences from controls.'}
if ( length(dxglist) > 0  )
...
nmstemp2=c("voi",     "n.x (CN,PD-,PD+)",  "anv.x",   "d.x.MCI", "d.x.Sp-", "d.x.Sp+",'sig.x','sig.y' )
...
nmstemp2=c("voi",    "n.y (CN,PD-,PD+)",  "anv.y",     "d.y",     "d.y.MCI", "d.y.Sp-", "d.y.Sp+",'sig.y', 'sig.x'  )
temp[,nmstemp2[nmstempw]]=temp[,nmstemp[nmstempw]]
...
#      if ( anvp <= nnhqth | anvpx <= nnhqth ) {
      ...
      c2viz = c("NP1DPRS23","updrs1_score48", "updrs3_score60")
...
```{r quicknnhpost,echo=FALSE,eval=TRUE,fig.width=12,fig.height=14,warning=FALSE,message=FALSE,fig.cap='Partial regression plots for SiMLR mapping between M3RI and PD symptomology.'}
if ( length(cogglist) >= 3 )
...
mycap=paste0('Table 7. Significant SiMLR IDP to clinical measurements: top-k for each score.')
