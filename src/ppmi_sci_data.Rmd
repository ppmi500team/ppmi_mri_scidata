...
ntokeep = 2950
temp = ppmi[ head( order(ppmi$T1Hier_resnetGrade,decreasing=T),ntokeep) , ]
ysel = temp$yearsbl <= 4.5 
temp = temp[ temp$studyName %in% c('PPMI','ADNI') & ysel &
#  ppmi$DXSubAsyn %in%  mylevsx0[c(1,2,3)]&  
#  ppmi$joinedDX %in%  mylevsx1[c(1,2,3)] &  
#  subtyper::fs( ppmi$joinedDX %in% c( "CN","PDLRRK2" ) ) &
  subtyper::fs( temp$joinedDX %in% c( "CN","PDLRRK2","PDSporadic" ) ),]
...
dolrrk2=FALSE
if ( dolrrk2 ) {
  temp$TDX = temp$DXSubAsyn
  temp$TDX[ temp$LRRK2_study == FALSE ] = NA
  mydx='PDLRRK2'
} else {
...
# temp$TDX[ temp$DXSubAsyn  == 'PDLRRK2Positive' ]='DX'
  temp$TDX[ temp$DXSubAsyn  == 'PDSporadicNegative' ]='DXPD-'
  temp$TDX[ temp$DXSubAsyn  == 'PDSporadicPositive' ]='DXPD+'
  mydx='PDSp'
# mydx='PDSpNeg'
...
rsfnames = c( 
  getNamesFromDataframe( c("rsf","LRAVG",num,'_falff'), temp, exclusions=xcl ),
  getNamesFromDataframe( c("rsf","Asym",num,'_falff'), temp, exclusions=xcl ) )
...
temp = ppmi[ head( order(ppmi$T1Hier_resnetGrade,decreasing=T),ntokeep) , ]
ysel = temp$yearsbl <= 4.5 
temp = temp[ temp$studyName %in% c('PPMI','ADNI') & 
  temp$joinedDX %in%  mylevsx2 &
  subtyper::fs(temp$AsynStatus == 'Positive') & ysel ,]
if ( myrez == "sr" ) {
...
temp = fillBaselineColumn( temp,
        c(dtinamesasymlr,rsfnames, rsfnamescnx, idpst1, nmidps, fdnames, cogs ), 
        'PATNO', 'yearsbl', 0, 
        fast=T, verbose=F )[[1]]
#########################################################
#########################################################
...
#    if ( n %% 10 == 0 ) print(stadfrsf[which.min(stadfrsf$anv),])
    ...
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+ (1|PATNO) + T1Hier_resnetGrade + joinedDX  + duration_yrs + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl )" )
#    cogformcommon=paste( cog  , "~  (1|PATNO) + commonSex + brainVolume_BL + ( yearsbl )")
    temp2 = temp[ !is.na( temp[,voi]) & temp$yearsbl < 2.85, ]
