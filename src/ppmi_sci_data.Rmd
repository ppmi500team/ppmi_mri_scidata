...
ppmitrimfn = paste0(rdir,'ppmi_idps.csv')
rgT1 = c( 0.8, 1.02 )[2]
...
dxsel = ppmitrim$joinedDX %in% c("CN",'ProdromalLRRK2','ProdromalGBA','ProdromalSporadic','SWEDDSporadic','PDLRRK2',"PDGBA")
agsel = ppmitrim$age_BL >= 50 & ppmitrim$age_BL <= 80
...
if ( ! exists("ppmitrimsave") & FALSE ) {
  ...
  presim=read_simlr_data_frames( path.expand(simfile), 
      c("t1","t1a", "dt", "dta", "rsf" ))
  ppmitrim = apply_simlr_matrices(  ppmitrim, presim, robust=FALSE )[[1]]
  simnames = apply_simlr_matrices(  ppmitrim, presim, robust=FALSE )[[2]]
...
We supplement these PPMI data with subjects from the Alzheimer's Disease Neuroimaging Initiative (ADNI) due to the relative dearth of longitudinal control data in PPMI.  As with PPMI, we attach ANTsPyMM IDPs to `ADNIMERGE_10Feb2024` using the `subtyper` function `merge_ADNI_antspymm_by_closest_date`.  Subjects from ADNI are included if they are controls with acceptable quality longitudinal neuroimaging. To balance the PPMI and ADNI control cohorts, the `subtyper` function `match_cohort_pair` is used, matching the 'ADNI' cohort with the 'PPMI' cohort based on subject age at baseline. 

We then train a regression model on the matched subjects to adjust imaging variables for systematic differences across the studies via MRI manufacturers ('GE', 'Philips', 'Siemens') and magnetic field strength. Subjects aged between 50 and 70 and belonging to asymptomatic groups are designated as training samples.  The regression map is learned and applied to each IDP throughout the full cohort using the `subtyper` function `adjustByCovariates`.  The purpose of this process is to mitigate the influence of different imaging protocols, ensuring that subsequent analyses are less confounded by these factors.  This approach has been used in practical studies of ADNI MRI data [@Risacher2017].  These merged and adjusted IDP data records are included in the file `ppmi_idps_trim.csv`. SiMLR derived variables are denoted `t1PC*` (left-right averaged T1w derived feature sets), `t1aPC*` (asymmetry-related T1w derived feature sets), `dtPC*` (left-right averaged DTI derived feature sets), `dtaPC*` (asymmetry-related DTI derived feature sets) and `rsfPC*` (resting connectivity) where the `*` varies from 1 to 100.  These SiMLR derived variables limit the multiple testing considerations to 100 variables because these are typically grouped together.  That is, a given PC set (referred to as $IDP^k_\text{M3RI}$) is included in a single model (e.g. if $k=1$, then $age \approx t1PC1 + t1aPC1 + dt1PC1 + dtaPC1 + rsfPC1$).  We use this approach in a technical validation section below.
...
fdnames = getNamesFromDataframe( "_FD_", ppmitrim  )
...
```{r quicklmdx,echo=FALSE,eval=!exists("mydogU"),fig.width=8,fig.height=12,warning=FALSE,message=FALSE,fig.cap='Partial regression plots for example regions that show omnibus level fwe significant differences from controls.'}
###################
###################
mydog$TDX[ mydog$joinedDX %in% c("CN")]='CN'
mydog$TDX[ mydog$joinedDX %in% c("MCI")]='MCI'
...
mydogUa=mydogU[ mydogU$studyName == 'ADNI' & mydogU$age_BL <= 71.0, ]
...
nms2viz = c("t1.vol.sncdp.", "t1.vol.bn.gp.gpidp.","dti.mean.md.uncinate.fasciculus.asym")
for ( voi in sample(nmstotest2) ) {
  gc()
  n=nrow(dxdfT1)+1
  vois = paste0( voi, c("_BL", "_delta" ) )
  othdxform=dxform
  selector=!is.na( mydogU[,voi]) & mydogU$yearsbl <= maxy & 
    mydogU$imaging_protocol %in% c( "1", "2","3") & 
    mydogU$TDX %in% c( "CN", "PDSp-","PDSp+", "MCI"  )

  longids = mydogU$subjectID[  mydogU$roundyears >= 1 ]
  baseids = mydogU$subjectID[  mydogU$roundyears == 0 ]
...
#    othdxform=gsub("studyName","1",dxform)
  ...
  longselect = selector & mydogU$subjectID %in% bothids
  temp2 = mydogU[ longselect, ]
  temp2$LEDD[ is.na( temp2$LEDD ) ] = 0
  temp2$duration_yrs[ is.na( temp2$duration_yrs ) ] = 0
  ...
  anvp = min( mycoffs[ grep("PD",rownames(mycoffs)) ,5])
  # suppressMessages( anova(bmdl,mdl)$Pr[2] )
  dxdfT1[n,'voiraw']=voi
  dxdfT1[n,'voi']=voi
  dxdfT1[n,'n.base']=length(unique(mydogUbl$commonID))
  dxdfT1[n,'n.long']=length(unique(temp2$commonID))
...
#    print(  knitr::kable( data.frame(mycoffs[-1,coffcols]), "latex", booktabs = TRUE,       caption=shorten_pymm_names(voi)) %>%      kableExtra::kable_styling(latex_options = c("striped", "scale_down")) )
    ...
    mygg0 = jtools::effect_plot( resultsX$full_model,  !!sym('TDX'), interval=T, facet.modx=F ) + ggtitle(paste("X:",voix)) + labs(y = NULL) + theme(legend.position = "none") 
#    mygg1=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=F ) + ggtitle(paste("L:",voi)) + labs(y = NULL) + theme(legend.position = "top") )
    mygg2=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=T ) + ggtitle(paste("L:",voix)) + labs(y = NULL) + theme(legend.position = "top") )
...
dxdfT1selcolsY = c( dxdfT1selcols[c(1,3,5)] , dxdfT1selcols[ grep("d.year",dxdfT1selcols,fixed=TRUE ) ] )
...
```{r quicklmcogB,echo=FALSE,eval=FALSE,fig.width=8,fig.height=3,warning=FALSE,message=FALSE,fig.cap='Partial regression interaction plots show a representative region for each of the MDS-UPDRS outcomes that we test.  Each panel demonstrates how different levels of the IDP (x-axis) lead to different amounts of change over time. For example, the slope of the relationship btween MD asymmetry in the internal capsule and change in MDS-UPDRS2 score increases over time with the largest differences occuring at the right side of the top right panel.  This indicates that subjects with higher levels of baseline MD asymmetry in this IDP progress more rapidly on this score.'}
signames = nmstotest2
stadfrsf=data.frame()
cogglist=list()
for ( voi in signames ) {
  ...
  for ( cog in cogtotest  ) {
    ...
    if ( antspymm_vartype( voi ) == "T1"  & FALSE ) {
      motnameu="T1wQC1+T1wQC2+T1wQC3+T1wQC4+"
      selector = selector & subtyper::fs( mydogp2$T1Hier_resnetGrade >= rgT1 )
    }
    if ( antspymm_vartype( voi ) == "rsfMRI" & FALSE )  {
      motnameu = paste( motname, "+rsfQC1+rsfQC2+rsfQC3+rsfQC4" )
      selector = selector & subtyper::fs( mydogp2$rsf1_ol_loop  <= 0.5 &
        mydogp2$rsfMRI_fcnxpro129_FD_mean <= 0.5 )
      }
    if ( antspymm_vartype( voi ) == "DTI"  & FALSE )  {
      motnameu = paste( motnamedt,  "+DTIQC1+DTIQC2+DTIQC3+DTIQC4" )
      selector = selector & subtyper::fs( mydogp2$DTI1_ol_loop  <= 0.5 &
        mydogp2$DTI_dti_FD_mean <= 5 )
    }

    temp2b = mydogp2[ selector & mydogp2$yearsbl == 0.0, ]
    covarsX='duration_yrs + LEDD + commonEdu + age_BL + commonSex'
    resultsX = lm_anv_p_and_d( temp2b, cog, voi, covarsX, predictoroperator='+', verbose=F )
    anvpx=resultsX$model_comparison$Pr[2]

    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+(1|commonID)+ ", covarsX, " + ( yearsbl )" )
    ...
    if ( nrow(temp2) > 10 ) {
      ...
      anvpx=resultsX$model_comparison$Pr[2]
      othercols=rownames(mycoffs)[ grep(voix,rownames(mycoffs))]
      ndf = length(unique(temp2$commonID))
      chnd = effectsize::t_to_d( mycoffs[ othercols, 't value'], c(ndf,ndf) )
      stadfrsf[n,'cog']=cog
      stadfrsf[n,'voi']=voix
      poptbl = table( table( temp2$commonID ) )
      stadfrsf[n,'n.x']=resultsX$n
      stadfrsf[n,'n.long']=ndf
      stadfrsf[n,'anv.x']=anvpx
      stadfrsf[n,'anv']=anvp
      othercolsnm=c("d.b","d.y")
      chnd[1,1]=resultsX$effect_sizes[1]
      ...
      pairstovisB = c(
        ...
        "rsf.sommotb.2.dopamine.bl",
        "dti.mean.md.post.int.cap.asym.bl" # # updrs3
        )
      pairstovisC = c(
        "updrs1_score", # updrs1 
        "updrs1_score",  # updrs1 
        "updrs2_score",# updrs2 
        "updrs2_score", # updrs3on 
        "updrs3_score_on" #  updrs3
        )
      ptv = paste0(pairstovisB,"_",pairstovisC)
  #     if (  anvp < 3e-4 ) {
      qth = 3e-4 # 0.05/length(signames)
      if (  paste0(voix,"_",cog) %in% ptv | anvp < qth | anvpx < qth ) {
        cogdel = paste0(cog,"_delta")
        anvp = insight::format_p( anvp, digits=4 )
        anvpx = insight::format_p( anvpx, digits=4 )
        anvp1 = insight::format_p( mycoffs[voix,'Pr(>|t|)'], digits=4 )
        anvp2 = anvp1 # insight::format_p( mycoffs[paste0("yearsbl:",voix),'Pr(>|t|)'], digits=4 )
        ttl=( paste(  voix, myrez, " + ", cog,  " : b", anvpx,  " : y", anvp ) )
        temp2$TDX=as.character( temp2$TDX )
        temp2$TDX[ temp2$TDX == 'PDGBA+']='GB+'
        temp2$TDX[ temp2$TDX == 'PDLRRK2+']='LR+'
        temp2$TDX[ temp2$TDX == 'PDSp+']='sp+'
        gglist = list()
        gglist[[length(gglist)+1]]=jtools::effect_plot( resultsX$full_model, !!sym(voi), interval=T, facet.modx=F ) + ggtitle(paste("X:",voix)) +  theme(legend.position = "none") 
        gglist[[length(gglist)+1]]=( interactions::interact_plot( mdl,  'yearsbl',!!sym(voix), interval=T, facet.modx=F ) + ggtitle(paste("L:",voix)) +  theme(legend.position = "none") )
        grid.arrange( grobs=gglist, nrow=1, top=ttl )
...
#  mypvs[ stadfrsf$cog != cog  ] = NA
  ...
  psel = psel | subtyper::fs( p.adjust( stadfrsf$anv.x, 'BY' ) <= sigth )
  psel2 = psel2 | subtyper::fs( p.adjust( stadfrsf$anv.x, 'BH' ) <= sigth )
...
if ( length( cogglist ) > 0 )
  grid.arrange( grobs = cogglist, nrow=2, top='Interactions between IDPs and MDS-UPDRS changes within PD.' )
###########################################################################
```

```{r quicknnh,echo=FALSE,eval=!exists("qdfsignnh"),fig.width=10,fig.height=5,warning=FALSE,message=FALSE,fig.cap='SiMLR mapping between M3RI and PD symptomology.'}
# ```{r quicknnh,echo=FALSE,eval=!exists("qdfsignnh"),fig.width=10,fig.height=12,warning=FALSE,message=FALSE,fig.cap='SiMLR mapping between M3RI and PD symptomology.'}
maxy=4.1
qdfcols =  c("cog", "voi" , 'n', "anv.x", "anv", 'sig', othercolsnm)
#########################################################
coggers = c("ess", "gds", "lns", "moca", "NP1APAT", "NP1ANXS", "NP1DPRS", "pigd", "rem", "quip", "scopa", "stai", "VLTANIM", "updrs1_score", "updrs2_score", "updrs3_score", "updrs_totscore" )
...
npc = ncol(presim[[1]])
# npc = 10
coggers = c( "updrs1_score", "updrs2_score", "updrs3_score"  )
mydogp = apply_simlr_matrices(  mydogp, presim, robust=FALSE, 
...
for ( voi in (1:npc) ) {
  for ( cog in (coggers) ) {
    ...
    selector = (mydogp$TDX %in% dxsel ) &  severity_selection &
      mydogp$yearsbl <= maxy & subtyper::fs( mydogp$T1Hier_resnetGrade >= rgT1 )
    covarsX="duration_yrs + LEDD + commonEdu + age_BL + commonSex + ( yearsbl )"
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+(1|commonID)+ ", covarsX )
    longids = mydogp$subjectID[  mydogp$roundyears >= 1 & selector ]
    baseids = mydogp$subjectID[  mydogp$roundyears == 0 & selector ]
    bothids = intersect( longids, baseids )
    temp2 = mydogp[ selector & mydogp$subjectID %in% bothids, ]
    temp2b=mydogp[ selector & mydogp$yearsbl==0,]
    vars3=paste0( names(presim), paste0( "PC", voi ))
    # for ( zz in vars3 ) temp2[,zz]=scale(rank(temp2[,zz]),T,T)
    vars3f=paste("(",paste(vars3,collapse='+'),")")
    newform = paste( cogformcommon, " * ", vars3f )
    resultsX = lm_anv_p_and_d( temp2b, cog, vars3f, covarsX, predictoroperator='+', verbose=F )
    anvpx=resultsX$model_comparison$Pr[2]
    if ( nrow(temp2) > 10 ) {
      temp2 = scale_variables_in_equation( temp2, newform , 
        variables_to_exclude=c('yearsbl',vars3))
      mdl=suppressMessages(lmer( newform, data=temp2 ))
      temp2=temp2[names(predict(mdl)),]
      bmdl=suppressMessages(lmer( cogformcommon, data=temp2 ))
      mdl=suppressMessages(lmer( newform, data=temp2 ))
      nlong = length(unique( temp2[names(predict(mdl)),'commonID']))
      mycoffs = coefficients( summary( mdl ) )
      anvp = suppressMessages( anova(bmdl,mdl)$Pr[2] )
      othercols=rownames(mycoffs)[ grep("PC",rownames(mycoffs))]
      ndf = length(unique(temp2$commonID))
      chnd = data.frame( 
        effectsize::t_to_d( mycoffs[ othercols, 't value'], c(ndf,ndf) ))
  ...
  #    stadnnh[n,'n.long']=poptbl[2]
      ...
      vars3xt=c(vars3,paste0("yearsbl:",vars3))
      vars3xt=intersect(vars3xt,rownames(mycoffs))
      xviz = names(which.min( mycoffs[vars3xt,"Pr(>|t|)" ] ))
      xviz = gsub("yearsbl:","",xviz)
      wmat = unlist( strsplit( xviz, 'PC') )
      n2show=2
      stadnnh[n,paste0("SiMLR",1:n2show)]=names(interpret_simlr_vector2( presim[[wmat[1]]] , paste0('PC',wmat[2]), n2show=n2show ))
      if ( anvp <= nnhqth | anvpx <= nnhqth ) {
#      if ( paste0(cog,voi) %in% c( "updrs1_score91", "updrs3_score31","updrs2_score64") ) {
        cogdel = paste0(cog,"_delta")
        anvp = insight::format_p( anvp, digits=4 )
        anvpx = insight::format_p( anvpx, digits=4 )
        anvp1 = insight::format_p( min(mycoffs[othercols,'Pr(>|t|)']), digits=4 )
        anvp2 = anvp1 # insight::format_p( mycoffs[paste0("yearsbl:",voix),'Pr(>|t|)'], digits=4 )
        ...
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=jtools::effect_plot( resultsX$full_model,  !!sym(vars3[k]), interval=T, facet.modx=F ) + ggtitle(paste("X:",vars3[k])) + theme(legend.position = "none") 
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=( interactions::interact_plot( mdl,  'yearsbl',!!sym(vars3[k]), interval=T, facet.modx=F ) + ggtitle(paste("L:",vars3[k]))  + theme(legend.position = "none") )
...
qdfsignnh=stadnnh[ stadnnh$sig %in% c("fwe"), ]
...
qdfsignnh$n = paste0(qdfsignnh$n.x,"/",qdfsignnh$n.long)
zzz=intersect(colnames(qdfsignnh),c(qdfcols,"SiMLR1","SiMLR2"))
zzz=c("cog","voi","n","anv.x","anv","SiMLR1","SiMLR2")
qdfsignnh = qdfsignnh[,zzz]
...
if ( FALSE )
  grid.arrange(grobs=list( 
    cogglist[[3]], cogglist[[2]], cogglist[[1]] 
  ),nrow=3)
###########################################################################
```


```{r updrsresults,echo=FALSE,eval=FALSE,warning=FALSE,message=FALSE,results='asis'}
# Print the ztable object, ensuring it's treated as LaTeX code
toviz = na.omit(qdfsig[qdfsig$sig=='fwe',])
toviz = head( toviz[ , !colnames(toviz) == 'sig'], 20 )
qdfsigz=ztable(toviz,zebra=2,tabular=F)
mycap='Table 7. Significant (fwe) IDP to MDS-UPDRS longitudinal effects for PD+: T1w, DTI and rsfMRI'
print(qdfsigz, type = "latex",size=3,caption=mycap,sidewaystable=TRUE)
```


```{r updrsresultsSiMLR,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE,results='asis'}
# Print the ztable object, ensuring it's treated as LaTeX code
...
nn=10
simlrviz=rbind( 
  head(qdfsignnh[qdfsignnh$cog=='updrs1',],nn),
  head(qdfsignnh[qdfsignnh$cog=='updrs2',],nn),
  head(qdfsignnh[qdfsignnh$cog=='updrs3',],nn) )
qdfsigzNNH=ztable( simlrviz, zebra=2,tabular=F)
mycap='Table 7. Significant (fwe) SiMLR IDP to MDS-UPDRS effects for PD+: top 10 for each score.'
print(qdfsigzNNH, type = "latex",size=3,caption=mycap,sidewaystable=TRUE)
```


We use LMMs to estimate the relationship of IDP values to clinical decline as evaluated by MDS-UPDRS 1, 2 and 3 (off) scores.  While these clinical measurements have well-documented limitations in terms of reliability and interpretability [@on_behalf_of_the_parkinsons_progression_markers_initiative_parkinsons_2023], they are consistently available in PPMI.  These exemplar assessments differ from the prior section in that they focus only on SAA$+$ sporadic PD subjects.  These models are of the form:

$$
\begin{split}
\text{UPDRSX}_{delta} \approx (1|ID) + \text{UPDRSX}_{bl} + \text{BV}_\text{bl} + Edu + LEDD + duration_\text{yrs}  + \\ 
  age_\text{bl} + Sex_\text{bio} + years_\text{bl}  *  IDP^k_\text{M3RI}
\end{split}
$$

The outcome is $\text{UPDRSX}_{delta}$ indicating change in a given MDS-UPDRS score.  The majority of these variables are as defined previously.  However, we introduce control for disease duration, treatment effects (LEDD) as well as baseline values of the given score.  This latter variable approximates a control for subject and domain-specific disease severity.  As such, these models are relatively conservative in terms of their attribution of variance to  IDP values.  The predictor of interest, here, is $years_\text{bl}  *  IDP^k_\text{M3RI}$ which estimates change in the given MDS-UPDRS score in relation to brain structure (including asymmetry) and function as measured by three modalities.  These models are assessed in the range of baseline to `r maxy` years change.  The effects are visualized through partial residual plots for select IDPs of interest.  Effect sizes and $p$-values for each significant (fwe) pair of outcomes and IDPs in Table 7 where SiMLR1 and SiMLR2 indicate the top two IDP features contributing to the model.  Overall, these studies demonstrate that baseline brain state -- as measured jointly by these modalities -- may contribute to acceleration/deceleration of changes in MDS-UPDRS scores.  However, we accentuate that these models are relatively simple and linear; as such, they yield only rough suggestions that additional modeling effort may be warranted to understand the differential value of these IDPs across the spectrum of PD symptomology.

<!--

### Multi-view example: M3RI predicts change in MDS-UPDRS1 in 2 to 4 year followup

Here, we dissect the influence of M3RI neuroimaging biomarkers on the progression of MDS-UPDRS Part I, II and III scores [@chahine_predicting_2019]. The analysis focuses on three neuroimaging biomarkers indicative of structural and functional brain integrity: asymmetry in the putamen volume as derived from T1-weighted images, motor cortex connectivity metrics from resting-state fMRI, and mean diffusivity measures from DTI in the superior longitudinal fasciculus. We construct three linear regression models that predict change in MDS-UPDRS1 from baseline data.  The base model uses only clinical information.  We build two additional IDP models: the first includes only the putamen IDP; the second includes IDPs from each modality. This last model extended the clinical reference model by incorporating all three neuroimaging biomarkers simultaneously, allowing for the assessment of their improvement in predicting motor symptom progression in the 2 to 4 year range of the followup data. The models include covariates such as LEDD, disease duration, baseline motor function, and demographic information.


```{r multiview,echo=FALSE,eval=FALSE,fig.width=10,fig.height=5,message=FALSE,warning=FALSE,fig.cap="Resampled estimates of multi-view regression for predicting change in MDS-UPDRS1 scores based on three different modalities demonstrate improved expected peformance.  Vertical dashed lines show median performance.",cache=TRUE}
...
$$
\begin{split}
\text{MDS-UPDRS1}_{delta} \approx  \cdots + years_\text{bl}  * \\
 (  \cdots ~+~rsf.sommota.2.contc.bl~+~dti.mean.md.fornix.asym.bl )
\end{split}
$$
Paired $t$-test show the 3 IDP model improves correlation between real and predicted MDS-UPDRS1 change scores versus both the baseline (`r insight::format_p(mvanvp3,digits=4)`) and 1 IDP model (`r insight::format_p(mvanvpu,digits=4)`). Figure 10 demonstrates the distribution of performance between the clinical and 3 IDP models over all of the simulations.  
...
ANTsPyMM is installable via `pypi` and available at [github](https://github.com/stnava/ANTsPyMM). The version used for this work is `1.4.` along with tensorflow `2.11.0`, antspyx `0.5.0`, antspynet `0.2.8` and antspyt1w `0.9.4`.

# Acknowledgements

Our sincere appreciation to the Michael J. Fox Foundation (MJFF) for supporting this work through MJFF-021144.

# Author contributions statement

B.A. conceptualized and designed the study. L.F., O.H., A.R., A.S., X.W. conducted the experiments and collected the data. B.A. and L.B. performed data analysis and interpretation. N.J.T and P.A.C. contributed to the development of the software framework and methodology. A.S. and A.R. were responsible for the software and computational tools used in the study. L.C., B.M., R.G, K.P. and K.M. supervised the research and provided critical feedback on the manuscript. B.A. wrote the first draft of the manuscript. All authors reviewed and approved the final version of the manuscript.
...
```{r simlr,echo=FALSE,eval=FALSE,fig.width=10,fig.height=5,message=FALSE,warning=FALSE,fig.cap="Resampled estimates of multi-view ML for predicting change in MDS-UPDRS scores based on three different modalities demonstrate improved expected peformance.  Vertical dashed lines show median performance.",cache=TRUE}
n.comp=100
simfile = paste0("~/Downloads/ppmi_pym_data/PPMISiMLRW_new_ica",n.comp)
if ( ! file.exists( paste0(simfile,'_t1_simlr.csv') ) ) {
    temp = ppmitrim[ppmitrim$yearsbl==0 & ppmitrim$studyName == 'PPMI',]
    stopifnot(nrow(temp)>10)
    trainer = na2f( temp$T1Hier_resnetGrade > 1.12 )
    print(table(trainer))
...
#        covariates=c( 'center', 'scale', 'lowrank', 'center', 'scale', 'np' ),
        covariates=c(  'center', 'scale',  'whiten', 'np' ),
        exclusions=c("fcnxpro134","fcnxpro122",'mean_md' ,'area','vol','adjusted','delta',"_BL","PC",'reference'),
        returnidps=FALSE,
        verbose=T )
