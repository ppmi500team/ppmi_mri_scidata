...
if ( ! exists("clin2b") ) {
  ...
  if ( ! exists("pymf") ) {
    pymfns = data.frame(
      fns=c( 
        paste0( rdir, 'ppmi_matched_qc_mm_processedCSV_v1.2.8.csv'),
        paste0( rdir, 'ppmi_matched_qc_mm_processedCSVSRFIRST_v1.2.8.csv'),
        paste0( rdir, 'ppmi_matched_qc_mm_processedCSVSRNext_v1.2.8.csv')  ),
      rez=c("OR","SRF","SRP") )
...
```{r ppmitrim,echo=FALSE,eval=!exists("ppmitrim"),fig.width=16,fig.height=10}
library(GGally)
...
psnrth=12.0
mmspc=2.0
evThresh=c(0.40,0.9)  # 0.3
rgThresh=1.02
if ( myrez != 'SRF' ) {
  snrth=c(5,30)
  snrth=c( quantile(ppmi$snr,0.01,na.rm=T) ,Inf)
} else {
  snrth=c( quantile(ppmi$snr,0.01,na.rm=T),Inf)
}
asel = subtyper::fs( ppmi$studyName == 'ADNI' & ppmi$T1Hier_resnetGrade_BL >= rgThresh & ppmi$T1Hier_resnetGrade >= 0.8 )
psel = subtyper::fs( ppmi$studyName %in% c('PPMI') & !(ppmi$subjectID == "101021" ) & 
    ppmi$SITE != 76 &
    ( ppmi$snr >= snrth[1] & ppmi$snr <= snrth[2] ) &
    ppmi$maxtominspc <= mmspc & ppmi$T1Hier_resnetGrade_BL >= rgThresh &
    ppmi$T1Hier_resnetGrade >= 0.8 &
    ppmi$mi < mith & ppmi$ssim > ssimth & ppmi$psnr > psnrth &
    ppmi$EVR >= evThresh[1] &  ppmi$EVR <= evThresh[2] )
ppmitrim = ppmi[ psel | asel, ]
...
#  subtyper::fs( ppmi$rsfMRI_fcnxpro122_FD_max <= 0.5 )
...
motnamedt = "1 + DTI_dti_fa_evr  + DTI_dti_FD_max  "
nmstotest = idpst1
mycorr = 'BY'
if ( is_test ) {
...
```{r predefregions1,echo=FALSE,eval=buildsci,warning=FALSE,message=FALSE,results='asis'}
...
```{r quicklmdx,echo=FALSE,eval=buildsci,fig.width=9,fig.height=12,warning=FALSE,message=FALSE,results='asis',fig.cap='Partial regression plots for example regions that show omnibus level fwe significant differences from controls.'}
###################
...
#  bform = paste( voi, " ~ brainVolume_BL+commonEdu+duration_yrs+LEDD+", othdxform )
  ...
  temp2 = mydogU[ !is.na( mydogU[,voi]) &  mydogU$studyName %in% c('PPMI','ADNI'), ]
...
#  if ( any(  mycoffs[ multigrep( c("PD","MCI"),rownames(mycoffs)), 5 ] < 1e-3 ) ) {
  ...
  if ( shorten_pymm_names(voi) %in%  thosetoviz ) {
...
#    print(  knitr::kable( data.frame(mycoffs[-1,coffcols]), "latex", booktabs = TRUE,       caption=shorten_pymm_names(voi)) %>%      kableExtra::kable_styling(latex_options = c("striped", "scale_down")) )
    ...
    dxglist[[length(dxglist)+1]]=interactions::probe_interaction(mdl, pred = 'yearsbl', modx = 'TDX',confint=F, interval = F)$interactplot + ggtitle( ttl ) + ylab( shorten_pymm_names(voi) )
...
mycap='Table 5. Time independent group effects in PPMI PD.'
...
for ( cog in  cogs[ -grep("tot",cogs)]  ) {
  for ( voi in nmstotest ) {
    ...
    voi = paste0(voi,"_BL")
    maxy=5.0
    if ( antspymm_vartype( voi ) == "T1" ) motnameu=""
    ...
    cogformcommon=paste( cog, "  ~ (1|PATNO) + ",motnameu," + AsynStatus + joinedDX  + duration_yrs + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl )" )
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+ (1|PATNO) + brainVolume_BL + ", motnameu, " + age_BL + commonSex + AsynStatus + yearsbl" )
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+", raneffstr, "+ duration_yrs + ",motnameu," + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl )*TDX + ( yearsbl )" )
...
#     if (  anvp < 3e-4 ) {
  ...
  #    print( voi )
      ...
      gg2 =interactions::probe_interaction(mdl, modx = 'yearsbl', pred = !!sym(voix) ,confint=F, interval = F)$interactplot 
      cogglist[[length(cogglist)+1]]=gg2
...
vois = c(
  "T1Hier_vol_bn_str_pu_Asymdeep_cit168_BL",
#  "T1Hier_vol_LRAVG_inferior_parietaldktcortex_BL",
  "rsfMRI_fcnxpro122_SomMotB_2_Dopamine_BL",
#  "rsfMRI_fcnxpro122_SomMotA_2_ContC_BL",
  "DTI_mean_md.fornix_.cres..stria_terminalis_.can_not_be_resolved_with_current_resolution..Asym.jhu_icbm_labels_1mm_BL")
...
raneffstr=''
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs + DTI_dti_FD_max_BL+ rsfMRI_fcnxpro122_FD_max_BL  + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl ) + ( yearsbl )  ")
...
#  mydogp$TDX != "CN" & mydogp$yearsbl <= 2.5, ]
for ( x in 1:length(vois) ) {
...
for ( jj in 1:n ) {
  ...
  mycoffs = coefficients( summary( mdl ) )
  mvanv = suppressMessages(anova(bmdl,mdl))
  mvanvp = mvanv$Pr[2]
...
```{r multiview2,echo=FALSE,eval=FALSE}
...
vois = c(
  "T1Hier_vol_LRAVG_inferior_parietaldktcortex_BL",
  "rsfMRI_fcnxpro122_SomMotB_2_ContA_BL",
  "DTI_mean_md.fornix_.cres..stria_terminalis_.can_not_be_resolved_with_current_resolution..Asym.jhu_icbm_labels_1mm_BL")
vois = c(
#  "DTI_mean_md.fornix_.cres..stria_terminalis_.can_not_be_resolved_with_current_resolution..Asym.jhu_icbm_labels_1mm_BL",
  "T1Hier_vol_LRAVG_inferior_parietaldktcortex_BL",
 # "DTI_mean_md.anterior_limb_of_internal_capsule.Asym.jhu_icbm_labels_1mm_BL"
  "T1Hier_vol_LRAVG_superior_parietaldktcortex_BL"
  )
voisx=vois
for ( x in 1:length(vois) ) {
  voisx[x]= shorten_pymm_names(voisx[x])
...
cogv=paste0(cog,c("_BL","_delta"))
raneffstr=''
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs + DTI_dti_FD_max_BL+ DTI_dti_FD_mean_BL  + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl ) + ( yearsbl )  ")
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl ) + ( yearsbl )  ")
uvform =paste( bform, "   *  ( ", voisx[1] , ")" )
mvform =paste( bform, " + ( yearsbl )  *  ( ", paste(voisx,collapse="+") , ")" )
temp2 = mydogp[  !is.na( mydogp[,vois[2]]) & 
#  mydogp$yearsbl <= maxy, ]
  mydogp$TDX == "PDSp+" & mydogp$yearsbl <= maxy, ]
#  mydogp$TDX != "CN" & mydogp$yearsbl <= 2.5, ]
for ( x in 1:length(vois) ) {
...
for ( jj in 1:n ) {
  ...
  mvanvp = mvanv$Pr[2]
  highy=train_test[[2]]$yearsbl >= 0.80 # & train_test[[2]]$yearsbl < 2.2
  highy=train_test[[2]]$yearsbl >= 1.80 & train_test[[2]]$yearsbl < 99
  train_test[[2]]=train_test[[2]][highy,]
  if ( nrow(train_test[[2]]) > 5 ) {
    mypredb = predict( bmdl, newdata=train_test[[2]])#, allow.new.levels = TRUE )
    mypredu = predict( mdlu, newdata=train_test[[2]])#, allow.new.levels = TRUE )
...
nna=!is.na( ppmi$rsfMRI_fcnxpro122_FD_mean )
