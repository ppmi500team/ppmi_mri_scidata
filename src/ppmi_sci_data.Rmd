...
###
toadd = c(  "age_BL", "commonSex", 'race',  'hy_BL', 'duration_yrs', "updrs_totscore")
...
names(joinedem)[4]=tblcols[4]='hy.stage'
names(joinedem)[5]=tblcols[5]='dur.yrs'
names(joinedem)[6]=tblcols[6]='updrs.total'
...
![Summary reproducibility results from aggregated traveling subject data. T1 IDPs represent high reproducibility in all categories (cerebellum, CIT168, cortical volume, cortical thickness, basal forebrain and medial temporal lobe).  DTI IDPs are also highly reproducible with FA in the cortical gray matter (gm) nearly equaling that of major white matter regions in the JHU atlas. Resting state default mode connectivity shows good reproducibility with the exception of default mode to subcortical connectivity where it is poor.](../figs/antspymm_repro.png)
...
cohortCCbl = cohortCCbl[ cohortCCbl$DXSubAsyn %in% c("CNNegative","PDSporadicPositive"), ]
cohortCCbl$DXbio = 0 #as.character( cohortCCbl$joinedDX )
cohortCCbl$DXbio[  cohortCCbl$DXSubAsyn == "PDSporadicPositive" ] = 1
...
idpst1 = idps[ grep("T1Hier",idps)]
idpsdt = idps[ multigrep(c("DTI","mean_fa"),idps)]
...
mycx = mycx[ !( mycx %in% c("classif.gbm", "classif.ksvm", "classif.fnn",  "classif.kknn", "classif.rpart", "classif.xgboost") ) ]
...
print( mycvagg )
#########################
g1=( ggstatsplot::ggbetweenstats( mycv, mdl, bacc, p.adjust.method = 'holm', pairwise.comparisons = F ) + ggtitle(paste("Bal. Acc. T1 + DTI: CN- vs sporadic PD+")))
g2=( ggstatsplot::ggbetweenstats( mycv, mdl, acc, p.adjust.method = 'holm', pairwise.comparisons = F ) + ggtitle(paste("Raw Acc. T1 + DTI: CN- vs sporadic PD+")))
