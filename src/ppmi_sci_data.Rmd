...
```{r quicklmdx,echo=FALSE,eval=!exists("mydogU"),fig.width=8,fig.height=7,warning=FALSE,message=FALSE,fig.cap='Partial regression plots for example regions that show omnibus level fwe significant differences from controls.'}
###################
###################
...
for ( voi in apriori_targets ) {
  ...
  selector=complete_cases_from_equation( mydogU, newform) & mydogU$yearsbl <= maxy & 
...
for ( voi in signames ) {
  ...
  for ( cog in cogtotest  ) {
    ...
    selector = !is.na( mydogp2[,voi]) & (mydogp2$TDX %in% c("PDSp+")) & 
      mydogp2$yearsbl <= maxy & subtyper::fs( mydogp2$T1Hier_resnetGrade >= FIXME )
    if ( antspymm_vartype( voi ) == "T1"  & FALSE ) {
      motnameu="T1wQC1+T1wQC2+T1wQC3+T1wQC4+"
      selector = selector & subtyper::fs( mydogp2$T1Hier_resnetGrade >= FIXME )
    }
...
if ( ! file.exists( paste0(simfile,'_t1_simlr.csv') ) ) {
    ...
    mycc = complete.cases( ppmitrim[,vars3])
...
for ( voi in (1:npc) ) {
  for ( cog in (coggers) ) {
    ...
    dxsel = c("PDSporadic","PDSNCA","PDLRRK2","PDGBA","MCI",
    ...
    mydogpnna=mydogp[ complete.cases( mydogp[,all.vars(as.formula(newform))] ) &
      (mydogp$TDX %in% dxsel ) & mydogp$yearsbl <= maxy,] 
    ...
    if ( cog=='MOCA') topkval=c(425,225) else topkval=c(375,200)
    selector = na2f( topk( mydogpnnaBase, "T1Hier_resnetGrade", topkval[1] ) )
    selectorLong = na2f( topk( mydogpnnaLong, "T1Hier_resnetGrade", topkval[2] ) )
    temp2b=mydogpnnaBase[ selector,]
    resultsX = lm_anv_p_and_d( temp2b, cog, vars3f, covarsX, predictoroperator='+', verbose=F )
    anvpx=resultsX$model_comparison$Pr[2]
    temp2 = mydogpnnaLong[ selectorLong, ]
    if ( nrow(temp2) > 10 ) {
      temp2 = scale_variables_in_equation( temp2, newform , 
...
zzz=intersect(colnames(qdfsignnh),c(qdfcols,"S_m1_1","S_m2_1"))
zzz=c("cog","voi","n","anv.x","anv","S_m1_1","S_m2_1")
...
simlrviz=rbind( 
  ...
  head(qdfsignnh[qdfsignnh$cog=='MOCA',],nn) )
