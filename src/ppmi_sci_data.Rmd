...
if ( file.exists( ppmitrimfn ) & ! exists("ppmitrim")) {
  ...
  nmstotest2=nmstotest2[ -multigrep( c("alff","peraf","template","fcnxpro134","fcnxpro122","_adjusted","_BL","_delta"), nmstotest2)]
#  nmstotest2=sample(nmstotest2,25)
...
#    mydogU$age_BL[mydogU$studyName=='PPMI'] )
# summary(glm( studyNameNum ~ age_BL * commonSex, data=mydogU))
mydogU$TDX[ mydogU$TDX %in% c("MCI-")]='MCI'
...
for ( voi in nmstotest2 ) {
  ...
  longids = mydogU$subjectID[  mydogU$yearsbl >= 1 ]
...
#  if ( (anvp < 0.001/length(nmstotest2) |  dxdfT1[n,'anv.x'] < 0.001/length(nmstotest2)) ) {
...
psel = subtyper::fs( p.adjust( dxdfT1$anv.x, mycorr ) <= .05 )
##########################################################
if ( TRUE ) {
...
mydogp = apply_simlr_matrices(  mydogp, presim, robust=FALSE )[[1]]
nnhnames = apply_simlr_matrices(  mydogp, presim, robust=FALSE )[[2]]
...
stadnnh=data.frame()
cogglist=list()
nnhqth = 0.01 / ( npc * length(coggers) )
for ( voi in (1:npc) ) {
  for ( cog in (coggers) ) {
    n = nrow(stadnnh)+1
    dxsel = c("PDSp+","CN")[1]
    selector = (mydogp$TDX %in% dxsel ) & 
...
grid.arrange(grobs=list( 
  cogglist[[3]], cogglist[[1]], cogglist[[2]] 
),nrow=3)
###########################################################################
```
...
mycap='Table 7. Significant (fwe) SiMLR IDP to MDS-UPDRS longitudinal effects for PD+: top 10 for each score.'
...
n.comp=100
simfile = paste0("~/Downloads/ppmi_pym_data/PPMISiMLRW",n.comp)
if ( ! file.exists( paste0(simfile,'_t1_simlr.csv') ) ) {
    temp = mydogp[mydogp$yearsbl==0,]
    trainer = subtyper::fs( temp$T1Hier_resnetGrade > 1.12 )
    print(table(trainer))
