...
```{r classified,echo=FALSE,eval=buildsci,fig.width=10,fig.height=8,message=FALSE,warning=FALSE,cache=FALSE}
############################
ysel = ppmitrim$yearsbl >= 0 & !is.na( ppmitrim$T1Hier_thk_r_i_iicerebellum) & !is.na(ppmitrim$T1Hier_volLRAVGi_iicerebellum)
ysel = ppmitrim$yearsbl == 0 & !is.na( ppmitrim$T1Hier_thk_r_i_iicerebellum) & !is.na(ppmitrim$T1Hier_volLRAVGi_iicerebellum)
...
selfl = !is.na( ppmitrim$T2Flair_flair_wmh_prior )
selfmri=!is.na(  ppmitrim$rsfMRI_fcnxpro122_VisPeri_2_SalVentAttnA) & 
  !is.na(ppmitrim$rsfMRI_fcnxpro122_ContA_2_ContA) & 
  subtyper::fs(ppmitrim$rsfMRI_fcnxpro122_FD_mean <= 0.5)
ppmitrim$DX = NA
ppmitrim$DX[ grep("CN",ppmitrim$joinedDX)]='AR'
ppmitrim$DX[ grep("Prodromal",ppmitrim$joinedDX)]='AR'
ppmitrim$DX[ grep("PD",ppmitrim$joinedDX)]='PD'
cohortCCbl = ppmitrim[ ysel, ]
cohortCCbldt = ppmitrim[ ysel & seldti, ]
cohortCCblrsf = ppmitrim[ ysel & seldti & selfmri , ]
dxv='joinedDX'
dxs = c("CN","PDSporadic")
dxv='DXSubAsyn'
...
idpst1 = c( 
  idps[ multigrep( c("T1Hier",'vol',"LRAVG"),idps,intersect=TRUE)],
...
idpsrsfmri = getNamesFromDataframe( c("rsfMRI_fcnxpro122","_2_"), ppmi )
...
mycvdt = mlr3classifiercv(
       cohortCCbldt,
       tcols = c("DXbio", 'sex','age', idpst1,idpsdt ),
       nrepeats = nrep,
       partrate = srate,
       dup_size = mysamp,
       balancing = mybal,
...
mycvrs = mlr3classifiercv(
       cohortCCblrsf,
       tcols = c("DXbio",'sex','age', idpst1,idpsdt,idpsrsfmri  ),
       nrepeats = nrep,
       partrate = srate,
       dup_size = mysamp,
       balancing = mybal,
...
# mygg = grid.arrange( grobs=list(g1t1,g1), nrow=2, top=paste('DTI + T1 features outperform T1 features alone in discriminating SAA confirmed', dxs[1], "vs",  dxs[2]))
print( mygg )

```


```{r interpretit,echo=FALSE,eval=TRUE,fig.width=9,fig.height=5}
library(ggpubr)
library(randomForestSRC)
datcol=c(idpst1, idpsdt, idpsrsfmri)
datcols=shorten_pymm_names( c(idpst1, idpsdt, idpsrsfmri) )
myx=cohortCCblrsf
colnames(myx)[  colnames(myx) %in% datcol ] = datcols
myx=myx[,c("DXbio",'age','commonSex', 'imaging_protocol', datcols )]
myx$commonSex=factor(myx$commonSex)
rfobj <- rfsrc(DXbio ~ ., data = myx)
newimp = data.frame( feature=names(vimp(rfobj)$importance), importance=as.numeric(vimp(rfobj)$importance ))
