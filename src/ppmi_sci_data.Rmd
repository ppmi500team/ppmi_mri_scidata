...
```{r ppmitrimX,echo=FALSE,eval=TRUE,fig.width=16,fig.height=10,message=FALSE,warning=FALSE,cache=FALSE}
library(GGally)
...
We also supplement these PPMI data with subjects from the Alzheimer's Disease Neuroimaging Initiative (ADNI) due to the relative dearth of longitudinal control data in PPMI.  Subjects from ADNI are included if they are controls with acceptable quality longitudinal neuroimaging. To balance the PPMI and ADNI control cohorts, the `subtyper` function `match_cohort_pair` is used, matching the 'ADNI' cohort with the 'PPMI' cohort based on subject age at baseline. The resultant matched demographic data is identified and extracted, combining the PPMI cohort with matched control ADNI subjects.

Following this, we train a regression model to adjust imaging variables for systematic differences across the studies, the primary MRI manufacturers ('GE', 'Philips', 'Siemens') and magnetic field strength. Subjects aged between 50 and 80 and belonging to asymptomatic gorups are designated as training samples.  The regression map is learned and applied to each IDP throughout the full cohort using the `subtyper` function `adjustByCovariates`.  The purpose of this process is to mitigate the influence of different imaging protocols, ensuring that subsequent analyses are less confounded by these factors.
...
#  if ( any(  mycoffs[ multigrep( c("PD","MCI"),rownames(mycoffs)), 5 ] < 1e-3 ) ) {
  ...
  if ( (anvp < 0.01/length(nmstotest2) |  dxdfT1[n,'anv.x'] < 0.01/length(nmstotest2)) ) {
...
for ( voi in signames ) {
  ...
  for ( cog in cogtotest  ) {
    ...
    covarsX='imaging_protocol + duration_yrs + LEDD + commonEdu + age_BL + commonSex'
...
```{r quicknnh,echo=FALSE,eval=!exists("qdfsignnh"),fig.width=10,fig.height=6,warning=FALSE,message=FALSE,fig.cap='NNH results.'}
#########################################################
...
#  voi = paste0(voi,"_BL")
  for ( cog in (coggers) ) {
    n = nrow(stadnnh)+1
    maxy=4.5
    selector = (mydogp$TDX %in% c("PDSp+","CN")) & 
      mydogp$yearsbl <= maxy & subtyper::fs( mydogp$T1Hier_resnetGrade >= rgT1 )
    covarsX="imaging_protocol + duration_yrs + LEDD + commonEdu + age_BL + commonSex + ( yearsbl )"
  ...
  #    stadnnh[n,'n.long']=poptbl[2]
      ...
      n2show=2
      stadnnh[n,paste0("NNH",1:n2show)]=names(interpret_simlr_vector2( presim[[wmat[1]]] , paste0('PC',wmat[2]), n2show=n2show ))
...
qdfsignnh = qdfsignnh[,c(qdfcols,"NNH1","NNH2")]
...
```{r updrsresultsNNH,echo=FALSE,eval=TRUE,warning=FALSE,message=FALSE,results='asis'}
# Print the ztable object, ensuring it's treated as LaTeX code
toviz = na.omit(qdfsignnh[qdfsignnh$sig=='fwe',])
toviz = toviz[ , !colnames(toviz) == 'sig']
qdfsigzNNH=ztable(toviz,zebra=2,tabular=F)
mycap='Table 8. Significant (fwe) IDP-UPDRS longitudinal effects for PD+: T1w, DTI and rsfMRI joint prediction.'
...
```{r simlr,echo=FALSE,eval=FALSE,fig.width=10,fig.height=5,message=FALSE,warning=FALSE,fig.cap="Resampled estimates of multi-view ML for predicting change in UPDRS scores based on three different modalities demonstrate improved expected peformance.  Vertical dashed lines show median performance.",cache=TRUE}
maxy=2.5
t1nms = paste0(nmstotest2[ grep("T1",nmstotest2)],"_BL")
dtnms = paste0(nmstotest2[ grep("DTI",nmstotest2)],"_BL")
rsnms = paste0(nmstotest2[ grep("rsfM",nmstotest2)],"_BL")
...
mysimVars = antspymm_simlr( mydogp, mydogp$yearsbl==0, 
    energy='cca', nsimlr=20, covariates='1', myseed=99, doAsym=2, 
    returnidps=TRUE, 
    exclusions=c("_delta","_BL"), 
    inclusions='_adjusted',
    verbose=TRUE)
mysim = antspymm_simlr( mydogp, mydogp$yearsbl==0, 
    energy='cca', nsimlr=12, covariates='1', myseed=808, doAsym=2, 
    returnidps=F, 
    exclusions=c("_delta","_BL",'_adjusted'), 
   # inclusions='_adjusted',
    verbose=TRUE)
