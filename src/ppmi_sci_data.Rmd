...
if ( ! file.exists( paste0(simfile,'_t1_simlr.csv') ) ) {
    ...
    temppca=prcomp( temp[,supervisors], scale. =TRUE, rank.=10 )
    colnames(temppca$x)=paste0('prcomp',1:10)
...
#      if ( paste0(cog,voi) %in% c( "updrs1_score91", "updrs3_score31","updrs2_score64") ) {
        ...
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=jtools::effect_plot( resultsX$full_model,  !!sym(vars3[k]), 
              interval=T, facet.modx=F, plot.points = F ) + ggtitle(paste("X:",vars3[k])) + theme(legend.position = "none") 
        gglist[[length(gglist)+1]]=m1[[2]]
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=( interactions::interact_plot( mdl,  'yearsbl', !!sym(vars3[k]), 
              interval=T, facet.modx=F , plot.points = F ) + ggtitle(paste("L:",vars3[k]))  + theme(legend.position = "none") )
