...
for ( voi in nmstotest ) {
  ...
  if ( antspymm_vartype(voi) == "rsfMRI" ) {
    othdxform=paste0(motname,"+",dxform)
    selector = selector & subtyper::fs( mydogU$rsf1_ol_loop  <= 0.5 &
      mydogU$rsfMRI_fcnxpro122_FD_mean <= 0.5 )
  }
...
for ( cog in  cogs[ -grep("tot",cogs)]  ) {
  for ( voi in nmstotest ) {
    ...
    if ( antspymm_vartype( voi ) == "rsfMRI" )  {
      motnameu = motname
      # maxy=2.5
      selector = selector & subtyper::fs( mydogp$rsf1_ol_loop  <= 0.5 &
        mydogp$rsfMRI_fcnxpro122_FD_mean <= 0.5 )
      }
    if ( antspymm_vartype( voi ) == "DTI" )  {
      motnameu = motnamedt
      # maxy=3.0
      selector = selector & subtyper::fs( mydogp$DTI1_ol_loop  <= 0.5 &
        mydogp$DTI_dti_FD_mean <= 5 )
    }
    selector = selector & !is.na( mydogp[,voi]) & mydogp$TDX != "CN" & mydogp$yearsbl <= maxy
