...
```{r nmratings,echo=FALSE,eval=FALSE}
...
ppmi500$NM_QC_Ratings_BA=NA
lohi=c(0.3,0.7)
ppmi500$NM_QC_Ratings_BA[ subtyper::fs( ppmi500$NM2DMT_NM_substantianigra_z_coordinate > lohi[1] &  ppmi500$NM2DMT_NM_substantianigra_z_coordinate < lohi[2] ) ]=1
ppmi500$NM_QC_Ratings_BA[ subtyper::fs( ppmi500$NM2DMT_NM_substantianigra_z_coordinate <= lohi[1] | ppmi500$NM2DMT_NM_substantianigra_z_coordinate >= lohi[2] ) ]=0
ppmi500$NM_QC_Ratings_BA[ subtyper::fs( ppmi500$NM2DMT_NM_volume_substantianigra < 500 ) ]=0
ppmi500$NM_QC_Ratings_BA[ subtyper::fs( ppmi500$NM2DMT_NM_avg_substantianigra > 2500 ) ]=0
ppmi500$NM_QC_Ratings_BA[ subtyper::fs( ppmi500$NM2DMT_NM_sd > 50 ) ]=0
ppmi500$NM_QC_Ratings_BA[ subtyper::fs( ppmi500$NM2DMT_NM_max > 5500 ) ]=0
table( ppmi500$NM_QC_Ratings_BA )
table( ppmi500$NM_QC_Ratings_BA)/sum(table(ppmi500$NM_QC_Ratings_BA))
```
...
```{r multiview,echo=FALSE,eval=buildsci2,fig.width=10,fig.height=5,message=FALSE,warning=FALSE,fig.cap="Resampled estimates of multi-view regression for predicting change in UPDRS1 scores based on three different modalities demonstrate improved expected peformance."}
library(groupdata2)
library(xpectr)
cog='updrs_totscore'
cog='updrs1_score'
...
raneffstr=''
bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + duration_yrs + DTI_dti_FD_mean_BL+ rsfMRI_fcnxpro122_FD_mean_BL  + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + ( yearsbl )*TDX + ( yearsbl )  ")
# bform =paste( cogv[2], " ~ ", cogv[1], " + ", raneffstr, "  + LEDD + age_BL + commonSex + ( yearsbl )  ")
...
\text{UPDRS1}_{delta} \approx  \cdots + years_\text{bl}  * \\
 (  \cdots ~+~rsf.sommota.2.contc.bl~+~dti.mean.md.sup.l.fasc )
\end{split}
$$
