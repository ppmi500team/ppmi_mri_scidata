...
```{r quicklmdx,echo=FALSE,eval=buildsci,fig.width=9,fig.height=6,warning=FALSE,message=FALSE}
######################################################################
...
dxform="  (1|subjectID) + imaging_protocol + age_BL + commonSex "
...
rsfnamescnx = c( 
  getNamesFromDataframe( c("rsfMRI_fcnxpro129","_2_","Vis","efaul"),  ppmi, exclusions=xcl ) )
...
ctx = c( "T1Hier_thk_Asym_middle_temporaldktcortex", "T1Hier_thk_Asym_inferior_parietaldktcortex", 
...
```{r dxresults,echo=FALSE,eval=buildsci,warning=FALSE,message=FALSE}
psel = subtyper::fs( p.adjust( dxdfT1$anv, mycorr ) <= .05 )
table(psel)
...
dxdfT1sel[, dxdfT1selcols] %>%
  knitr::kable("latex", booktabs = TRUE, caption='Significant longitudinal diagnostic group effects: T1w, DTI and rsfMRI (n.s.)') %>%
  kableExtra::kable_styling(latex_options = c("striped", "scale_down"))
```
...
```{r quicklmcog,echo=FALSE,eval=buildsci,fig.width=9,fig.height=3.5,warning=FALSE,message=FALSE}
#########################################################
...
for ( cog in  cogs  ) {
  for ( voi in nmstotest ) {
    ...
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+ (1|PATNO) + T1Hier_resnetGrade  + duration_yrs + ",motname," + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + DTI_mean_fa_mtg_sn_snc_LRAVG_BL + T1Hier_vol_mtg_sn_snc_LRAVGdeep_cit168_BL + TDX + ( yearsbl )" )
#    cogformcommon=paste( cog  , "~  (1|PATNO) + commonSex + brainVolume_BL + ( yearsbl )")
    ...
    newform = paste(  cogformcommon, " * TDX* ", voi )
    if ( nrow(temp2) > 100 ) {    
    temp2 = scale_variables_in_equation( temp2, newform )
...
#      print( mycoffs[ , -c(1:3)] )
  #    print( voi )
      ...
      ttl=( paste( voix, myrez, " + ", cog,  " : ", anvp ) )
      suppressMessages(
      print( threewayinteraction(  temp2, 'yearsbl', cog, 'TDX', voi, anatshow=voi ) ))
...
for ( cog in cogs ) {
  ...
  psel = subtyper::fs( p.adjust( mypvs, 'BY' ) <= 0.05 )
  psel2 = subtyper::fs( p.adjust( mypvs, 'BH' ) <= 0.05 )
...
qdf = qdf[ order( qdf$anv ), ]
qdf$voi = shorten_pymm_names( qdf$voi )
###########################################################################
if ( FALSE ) {
  stadfrsf$modality="T1w"
  stadfrsf$modality[ grep("DTI", stadfrsf$voi)]="DTI"
  stadfrsf$modality[ grep("rsfMRI", stadfrsf$voi)]="rsfMRI"
  qdf2=data.frame()
  for ( mod in c("T1w","DTI","rsfMRI") ) {
    mypvs = stadfrsf$anv
    mypvs[ stadfrsf$modality != mod  ] = NA
    psel = subtyper::fs( p.adjust( mypvs, mycorr ) <= 0.05 )
    siganat = stadfrsf$voi[ psel ]
    if ( sum(psel) > 0 )
      qdf2 = rbind( qdf2, stadfrsf[psel,] )
    }
  qdf2$voi = shorten_pymm_names( qdf2$voi )
}
###########################################################################
###########################################################################
```
...
```{r updrsresults,echo=FALSE,eval=buildsci,warning=FALSE,message=FALSE}
qdf$anv = insight::format_p(  qdf$anv, digits=4  )
qdfcols =  c("cog", "voi" , "anv", "sig" )
qdf = qdf[,qdfcols]
qdf = qdf[duplicated(qdf),]
knitr::kable(qdf, "latex", booktabs = TRUE, caption='Significant IDP-UPDRS longitudinal  effects: T1w, DTI and rsfMRI') %>%
  kableExtra::kable_styling(latex_options = c("striped", "scale_down"))
```
...
```{r quicklmdxDT,echo=FALSE,eval=FALSE,fig.width=9,fig.height=3.5,warning=FALSE,message=FALSE}
...
```{r dxresultsDT,echo=FALSE,eval=FALSE,warning=FALSE,message=FALSE}
###########################################################
...
```{r quicklmcogset,echo=FALSE,eval=FALSE,fig.width=9,fig.height=4.0,warning=FALSE,message=FALSE}
####################################################################
mydogp = mydog[ mydog$studyName == 'PPMI', ]
...
mylevsx2=c(  "CN", "ProdromalSporadic", "PDLRRK2", "PDGBA", "PDSporadic" )
nmidps = getNamesFromDataframe( c("NM2DMT"), mydogp, 
...
```{r quicklmcogfill,echo=FALSE,eval=FALSE,fig.width=9,fig.height=4.0,warning=FALSE,message=FALSE}
dtinames = getNamesFromDataframe( c("DTI_"), ppmi, exclusions=c( xcl, 'cnx','volume') )
fdnames = getNamesFromDataframe( "_FD_", ppmi  )
# dxvar = 'joinedDX'
dtinamesasymlr = c( 
  getNamesFromDataframe( c("DTI","LRAVG"), mydogp, exclusions=c('_BL',"_delta" )),
  getNamesFromDataframe( c("DTI","Asym"), mydogp, exclusions=c('_BL',"_delta") ) )
#######################################################
rsfnames = c( 
...
#######
fdnames = getNamesFromDataframe( "_FD_", mydogp  )
cogs = getNamesFromDataframe( c("updrs","score"), mydogp, exclusions=c(4,'_BL',"_delta") )
...
```{r quicklmcogrsf,echo=FALSE,eval=FALSE,fig.width=9,fig.height=4.0,warning=FALSE,message=FALSE}
#########################################################
# for ( voi in sample( c(rsfnames,dtinamesasymlr) ) ) {
stadfrsf=data.frame()
...
```{r cogresults,echo=FALSE,eval=FALSE,warning=FALSE,message=FALSE}
...
```{r lmerextab,echo=FALSE,eval=FALSE,fig.width=12,fig.height=6,message=FALSE,warning=FALSE,cache=FALSE}
