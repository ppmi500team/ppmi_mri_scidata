...
```{r classified,echo=FALSE,eval=buildsci,fig.width=6,fig.height=9,message=FALSE,warning=FALSE,cache=FALSE}
############################
...
cohortCCbldt = ppmi[ ysel & seldti, ]
cohortCCblrsf = ppmi[ ysel & seldti & selfmri , ]
dxv='joinedDX'
dxs = c("CN","PDSporadic")
dxv='AsynStatus'
...
idpst1 = c( 
  idps[ multigrep( c("T1Hier","LRAVG"),idps,intersect=TRUE)],
  idps[ multigrep( c("T1Hier","Asym"),idps,intersect=TRUE)],
...
mycx = mycx[ !( mycx %in% c("classif.gbm", "classif.rpart") ) ]
...
if ( FALSE ) {
  for ( z in c(idpst1,idpsdt,idpsrsfmri) ) {
    ...
    cohortCCbl = adjustByCovariates(cohortCCbl,loform,groupVariable='DXbasic',group='CN')
    cohortCCbldt = adjustByCovariates(cohortCCbldt,loform,groupVariable='DXbasic',group='CN')
    cohortCCblrsf = adjustByCovariates(cohortCCblrsf,loform,groupVariable='DXbasic',group='CN')
  }
...
cohortCCbldt$sex=0
cohortCCbldt$sex[cohortCCbldt$commonSex=='Male']=1
cohortCCblrsf$sex=0
cohortCCblrsf$sex[cohortCCblrsf$commonSex=='Male']=1
#######
# cohortCCbl=cohortCCbldt=cohortCCblrsf
cohortCCbl$study = 0
...
mycvt1 = mlr3classifiercv(
       cohortCCbl,
       tcols = c("DXbio", 'sex','age_BL', idpst1) ,
       nrepeats = nrep,
       partrate = srate,
       dup_size = mysamp,
       balancing = mybal,
...
mycvdt = mlr3classifiercv(
       cohortCCbldt,
       tcols = c("DXbio", 'sex','age_BL', idpst1, idpsdt ),
       nrepeats = nrep,
       partrate = srate,
       dup_size = mysamp,
       balancing = mybal,
...
myx=myx[,c("DXbio",'age_BL','commonSex', 'imaging_protocol', datcols )]
...
```{r interpretit2,eval=TRUE,echo=FALSE,fig.width=12,fig.height=4,message=FALSE,warning=FALSE}
library(ggpubr)
library(randomForestSRC)
datcol=c(idpst1, idpsdt, idpsrsfmri)
datcol=c(idpst1 )
...
colnames(myx)[  colnames(myx) %in% datcol ] = datcols
opreds = c('age_BL','commonSex', 'study', datcol )
myx=myx[,c("DXbio",'age_BL','commonSex', 'study', datcols )]
myx$commonSex=factor(myx$commonSex)
# rfobj <- rfsrc(DXbio ~ ., data = myxtr)
# rfobj <- rfsrc.fast(DXbio ~ ., data = myxtr)
mypartition = dataPartition( myx$age_BL, 0.8  )
myx$DXbio = factor( myx$DXbio )
# myrf=rfsrc( DXbio ~ ., data=myx[mypartition$train,], importance=T, forest=TRUE  )
myrf=imbalanced( DXbio ~ ., data=myx[mypartition$train,], importance=T, ntree = 5000  )
...
```{r interpretit2auc,eval=TRUE,echo=FALSE,fig.width=10,fig.height=6,message=FALSE,warning=FALSE,results='asis'}
## ggplot it
library(pROC)
...
```{r quicklmcogset,echo=FALSE,eval=buildsci,fig.width=9,fig.height=4.0,warning=FALSE,message=FALSE}
####################################################################
...
```{r quicklmcogfill,echo=FALSE,eval=buildsci,fig.width=9,fig.height=4.0,warning=FALSE,message=FALSE}
dtinames = getNamesFromDataframe( c("DTI_"), ppmi, exclusions=c( xcl, 'cnx','volume') )
fdnames = getNamesFromDataframe( "_FD_", ppmi  )
# dxvar = 'joinedDX'
dtinamesasymlr = c( 
