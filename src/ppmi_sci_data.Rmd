...
* reference biomedical data organization
...
Components of technical validity that are critical for quantitative methodology in neuroimaging include: (a) generally robust performance across domains; (b) multi-site reproducibility; (c) disease-specific discrimination from controls in particular over time in the clinical trial setting; (d) sensitivity to or relationship with changes in clinically relevant symptoms at baseline and/or over time.  

We further this by demonstrating ANTsX reproducibility and reliability through analysis of three traveling subject cohorts which provide evidence of reliability for multisite trials. By aggregating data from the traveling subject cohorts, we offer precise, reproducible reliability estimates (via intra-class correlation) manifested across different scanner types and imaging modalities.  These studies are followed by demonstrations (or examples) of statistical models that test established hypotheses comparing genetic and sporadic PD versus control subjects.  We finalize the technical validity section with examples of how scientists may relate baseline brain health to rate of symptom change in PD in an interpretable and modality-specific manner.  The scale of the current data supports control for a subset of important PD relevant covariates including disease duration, educational level, sex, age and levodopa dose equivalent daily dose.  These variables are included in reference models with additional details below.

The following sections address these primary domains of validity.

## Robust performance

The technical validity of these methods is supported by previous work, including various open quantitative MRI analysis challenges FIXME. 
