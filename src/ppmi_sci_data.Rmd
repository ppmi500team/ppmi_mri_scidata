...
if ( file.exists( ppmitrimfn ) & ! exists("ppmitrim")) {
  ...
  nmstotest2=nmstotest2[ -multigrep( c("alff","peraf","template","fcnxpro134","fcnxpro122","_adjusted"), nmstotest2)]
...
for ( voi in signames ) {
  ...
  #     if (  anvp < 3e-4 ) {
      ...
      if (  paste0(voix,"_",cog) %in% ptv | anvp < qth | anvpx < qth ) {
        ...
        gglist = list()
        gglist[[length(gglist)+1]]=jtools::effect_plot( resultsX$full_model, !!sym(voi), interval=T, facet.modx=F ) + ggtitle(paste("X:",voix)) + labs(y = NULL) + theme(legend.position = "none") 
        gglist[[length(gglist)+1]]=( interactions::interact_plot( mdl,  'yearsbl',!!sym(voix), interval=T, facet.modx=F ) + ggtitle(paste("L:",voix)) + labs(y = NULL) + theme(legend.position = "none") )
...
```{r quicknnh,echo=FALSE,eval=FALSE,fig.width=10,fig.height=6,warning=FALSE,message=FALSE,fig.cap='NNH results.'}
#########################################################
...
#  voi = paste0(voi,"_BL")
  ...
  #    stadnnh[n,'n.long']=poptbl[2]
      ...
      if (  anvp <= 0.05/npc | anvpx <=0.05/npc ) {
        cogdel = paste0(cog,"_delta")
        anvp = insight::format_p( anvp, digits=4 )
        anvpx = insight::format_p( anvpx, digits=4 )
        ...
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=jtools::effect_plot( resultsX$full_model,  !!sym(vars3[k]), interval=T, facet.modx=F ) + ggtitle(paste("X:",vars3[k])) + labs(y = NULL) + theme(legend.position = "none") 
        for ( k in 1:length(vars3)) 
            gglist[[length(gglist)+1]]=( interactions::interact_plot( mdl,  'yearsbl',!!sym(vars3[k]), interval=T, facet.modx=F ) + ggtitle(paste("L:",vars3[k])) + labs(y = NULL) + theme(legend.position = "none") )
        grid.arrange( grobs=gglist, nrow=2, top=ttl )
        print( mycoffs[-1,c(4:5)])
        print(stadnnh[n,] )
    }
...
qdfsig=stadnnh[ stadnnh$sig %in% c("fdr","fwe"), ]
qdfsig = qdfsig[ order( qdfsig$anv ), ]
qdfsig$anv.x = insight::format_p(  qdfsig$anv.x, digits=4  )
qdfsig$anv = insight::format_p(  qdfsig$anv, digits=4  )
qdfcols =  c("cog", "voi" , 'n.base', 'anv.x',  "anv", 'd.b', 'd.y', 'sig', othercolsnm)
qdfsig = qdfsig[,qdfcols]
qdfsig$voi=gsub("_",".",qdfsig$voi,fixed=TRUE)
qdfsig$cog=gsub("_",".",qdfsig$cog,fixed=TRUE)
qdfsig = qdfsig[!duplicated(qdfsig),]
