...
We supplement these PPMI data with subjects from the Alzheimer's Disease Neuroimaging Initiative (ADNI) due to the relative dearth of longitudinal control data in PPMI.  An additional advantage is the ability to compare PD subgroups with mild cognitive impairment (MCI) in terms of IDP effects both longitudinally and cross-sectionally relative to controls.  As with PPMI, we attach ANTsPyMM IDPs to `ADNIMERGE_10Feb2024` using the `subtyper` function `merge_ADNI_antspymm_by_closest_date`.  Subjects from ADNI are included if they are controls or MCI with acceptable quality neuroimaging. The `subtyper` function `match_data_frames` is used to balance the PPMI and ADNI cohorts for the purposes of statistical testing.  The function selects n=2150 subjects from the 'ADNI' cohort to match the 'PPMI' cohort based on subject age at baseline, sex and the Montreal Cognitive Assessment (MOCA).  ADNI subjects included here were no older than 71 years at baseline.  The ADNI cohort remained slightly older (mean of 64.1 years versus 63.7 in PPMI $p=0.05$ two sample $t$-test) even after matching.

We also train a regression model on the matched subjects to adjust imaging variables for systematic differences across the studies via MRI manufacturers ('GE', 'Philips', 'Siemens') and magnetic field strength. Control subjects aged between 50 and 70 are designated as training samples.  The regression map is learned and applied to each IDP throughout the full cohort using the `subtyper` function `adjustByCovariates`.  The purpose of this process is to mitigate the influence of different imaging protocols, ensuring that subsequent analyses are less confounded by these factors.  This approach has been used in practical studies of ADNI MRI data [@Risacher2017].  These merged and adjusted IDP data records are included in the file `ppmi_idps_trim_v1.4.0_SRF.csv`. SiMLR derived variables are denoted `t1PC*` (left-right averaged T1w derived feature sets), `t1aPC*` (asymmetry-related T1w derived feature sets), `dtPC*` (left-right averaged DTI derived feature sets), `dtaPC*` (asymmetry-related DTI derived feature sets) and `rsfPC*` (resting connectivity) where the `*` varies from 1 to 100.  These SiMLR derived variables limit the multiple testing considerations to 100 variables because these are typically grouped together.  That is, a given PC set (referred to as $IDP^k_\text{M3RI}$) is included in a single model (e.g. if $k=1$, then $age \approx t1PC1 + t1aPC1 + dt1PC1 + dtaPC1 + rsfPC1$).  We use this approach in a technical validation section below.
...
nmstemp2=c("voi",     "n.x",  "anv.x",   "d.x.MCI", "d.x.Sp-", "d.x.Sp+",'sig.x','sig.y' )
temp[,nmstemp2]=temp[,nmstemp]
temp[is.na(temp$sig.y),'sig.y']='n.s.'
dxdfT1selz=ztable( temp[temp$sig.x %in% c("fdr.x","fwe.x"),nmstemp2], zebra=2)
...
mycap='Table 5. Time independent group effects (fdr $p <= 0.05$) in PPMI/ADNI. Top-k per modality.'
...
nmstemp2=c("voi",    "n.y",  "anv.y",     "d.y",     "d.y.MCI", "d.y.Sp-", "d.y.Sp+",'sig.y', 'sig.x'  )
temp[,nmstemp2]=temp[,nmstemp]
temp[is.na(temp$sig.x),'sig.x']='n.s.'
dxdfT1selz=ztable( temp[ temp$sig.y %in% c("fdr.y","fwe.y"),nmstemp2],zebra=2)
...
mycap='Table 6. Longitudinal group effects (fdr $p <= 0.05$) in PPMI/ADNI. Top-k per modality.'
print(dxdfT1selz, type = "latex",size=2,caption=mycap,sidewaystable=TRUE)
```
...
mycap=paste0('Table 7. Significant (fdr $p$ <= 0.05) SiMLR IDP to clinical measurements: top-k for each score.')
print(qdfsigzNNH, type = "latex",size=3,caption=mycap,sidewaystable=TRUE)
```


We use LMMs to estimate the relationship of IDP values to clinical decline as evaluated by MDS-UPDRS 1, 2, 3 (off) and total scores.  While these clinical measurements have well-documented limitations in terms of reliability and interpretability [@on_behalf_of_the_parkinsons_progression_markers_initiative_parkinsons_2023], they are consistently available in PPMI.  These exemplar assessments differ from the prior section in that they focus only on PPMI subjects as these measurements are absent in ADNI.  These models are of the form:

$$
\begin{split}
\text{UPDRSX}_{delta} \approx (1|ID) + (1|Site) + \text{UPDRSX}_{bl} + \text{BV}_\text{bl} + Edu + LEDD + duration_\text{yrs}  + \\ 
  age_\text{bl} + Sex_\text{bio} + years_\text{bl}  *  IDP^k_\text{M3RI}
\end{split}
$$

The outcome is $\text{UPDRSX}_{delta}$ indicating change in a given MDS-UPDRS score.  The majority of these variables are as defined previously.  However, we introduce control for disease duration, treatment effects (Levodopa Equivalent Daily Dose, LEDD) as well as baseline values of the given score.  This latter variable approximates a control for subject and domain-specific disease severity.  As such, these models are relatively conservative in terms of their attribution of variance to  IDP values.  The predictor of interest, here, is $years_\text{bl}  *  IDP^k_\text{M3RI}$ which estimates change in the given MDS-UPDRS score (or derived score / subscore) in relation to brain structure (including asymmetry) and function as measured by three modalities.  These models are assessed in the range of baseline to `r maxy` years change.  The effects are visualized through partial residual plots for select IDPs of interest.  Effect sizes for each significant (fdr or fwe) pair of outcomes and IDPs in Table 7 where SiMLR1 and SiMLR2 indicate the top two IDP features contributing to the model.  

This SiMLR study demonstrates that brain state -- as measured jointly by these modalities -- may contribute to acceleration/deceleration of changes in MDS-UPDRS scores.  However, we accentuate that these models are relatively simple and linear; as such, they yield only rough suggestions that additional modeling effort may be warranted to understand the differential value of these IDPs across the spectrum of PD symptomology.  Furthermore, this analysis grouped all PPMI subjects together, includign controls; as such, it lacks the specificity that may be needed to parse subgroup specific relationships or those that only occur within specific stages of PD.  Despite these limitations, Table 7 demonstrates that all three modalities studied here influence symptom progression.  Furthermore, several of these models indicate that multiple IDPs are changing in concert with symptom progression in particular for NP1DPRS (presence of depressed moods from MDS-UPDRS I) and the MDS-UPDRS III total score.  Change in both of these associate with IDPs across all three modalities.  Variables assessed but not reaching significance include MOCA, NP1APAT (apathy) and REM sleep disturbance.
