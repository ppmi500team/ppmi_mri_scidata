...
###
toadd = c(  "age_BL", "commonSex", 'race',  'hy_BL', 'duration_yrs', "updrs_totscore", 'imaging_protocol')
...
print(sttblPD, type = "latex",size=4,caption="Table 1. Baseline cohort for subjects with T1w IDPs.",sidewaystable=TRUE)
...
```{r classified,echo=FALSE,eval=buildsci,fig.width=10,fig.height=16,message=FALSE,warning=FALSE}
library(mlr3learners)
library(ANTsR)
library(mlr3verse)
############################
dtvec = ppmitrim$DTI_mean_fa.body_of_corpus_callosum.jhu_icbm_labels_1mm
seldti = subtyper::fs( dtvec >= quantile(dtvec,0.005, na.rm=TRUE) )
selfmri=!is.na(  ppmitrim$rsfMRI_fcnxpro122_VisPeri_2_SalVentAttnA)
cohortCCbl = ppmitrim[ 
  ppmitrim$yearsbl == 0 & 
  seldti & 
  !is.na( ppmitrim$T1Hier_thk_r_i_iicerebellum) , ]
dxs = c("PDSporadicNegative","PDSporadicPositive")
dxs = c("CNNegative","PDSporadicPositive")
cohortCCbl = cohortCCbl[ cohortCCbl$DXSubAsyn %in% dxs , ]
cohortCCbl$DXbio = 0 #as.character( cohortCCbl$joinedDX )
cohortCCbl$DXbio[  cohortCCbl$DXSubAsyn == dxs[2] ] = 1
idps = antspymm_predictors(cohortCCbl, TRUE, TRUE )
idps = idps[ -grep("rsfMRI_fcnxpro129",idps)]
idps = idps[ -grep("rsfMRI_fcnxpro134",idps)]
idps = idps[ -grep("rsfMRI_fcnxpro122",idps)]
idps = idps[ -grep("unclass",idps)]
idps = idps[ -grep("T1Hier_RandBasis",idps)]
# idps = idps[ -grep("DTI_mean_md",idps)]
idps = idps[ multigrep( c("Asym","LRAVG"), idps ) ]
idpst1 = idps[ multigrep( c("T1Hier",'vol','LRAVG'),idps,intersect=TRUE)]
idpsdt = unique( c( 
    idps[ multigrep(c("DTI","mean_fa","LRAVG"),idps,intersect=TRUE)] ) )
#    idps[ multigrep(c("DTI","mean_md","LRAVG"),idps,intersect=TRUE)] ) )
cohortCCbl[,idps]=antsrimpute(cohortCCbl[,idps])
...
mycv = mlr3classifiercv(
       cohortCCbl,
       tcols = c("DXbio", idpst1, idpsdt ),
       nrepeats = nrep,
       partrate = srate,
       dup_size = mysamp,
       balancing = mybal,
...
mycvagg = aggregate( bacc ~ mdl, mycvt1, mean )
print( mycvagg )
mycvagg = aggregate( bacc ~ mdl, mycv, mean )
print( mycvagg )
mycvagg = aggregate( acc ~ mdl, mycv, mean )
print( mycvagg )
#########################
g1t1=( ggstatsplot::ggbetweenstats( mycvt1, mdl, bacc, p.adjust.method = 'holm', pairwise.display ='none' ) + ggtitle(paste("Bal. Acc. T1 alone: CN- vs sporadic PD+")))
g2t1=( ggstatsplot::ggbetweenstats( mycvt1, mdl, acc, p.adjust.method = 'holm', pairwise.display ='none') + ggtitle(paste("Raw Acc. T1 alone: CN- vs sporadic PD+")))
g1=( ggstatsplot::ggbetweenstats( mycv, mdl, bacc, p.adjust.method = 'holm', pairwise.display ='none' ) + ggtitle(paste("Bal. Acc. T1 + DTI: CN- vs sporadic PD+")))
g2=( ggstatsplot::ggbetweenstats( mycv, mdl, acc, p.adjust.method = 'holm', pairwise.display ='none' ) + ggtitle(paste("Raw Acc. T1 + DTI: CN- vs sporadic PD+")))

mygg = grid.arrange( grobs=list(g1t1,g1,g2t1,g2), nrow=4, top='DTI + T1 features outperform T1 features alone in discriminating SAA confirmed PD from controls.')
# mygg = grid.arrange( grobs=list(g1t1,g1), nrow=2, top='DTI + T1 features outperform T1 features alone in discriminating SAA confirmed PD from controls.')
print( mygg )
...
```{r quicklmdx,echo=FALSE,eval=buildsci,fig.width=9,fig.height=3.5,warning=FALSE,message=FALSE}
######################################################################
...
```{r quicklmdxDT,echo=FALSE,eval=buildsci,fig.width=9,fig.height=3.5,warning=FALSE,message=FALSE}
...
```{r dxresultsDT,echo=FALSE,eval=buildsci,warning=FALSE,message=FALSE}
###########################################################
...
```{r quicklmcog,echo=FALSE,eval=buildsci,fig.width=9,fig.height=4.0,warning=FALSE,message=FALSE}
####################################################################
...
```{r cogresults,echo=FALSE,eval=buildsci,warning=FALSE,message=FALSE}
