...
```{r quicklmdx,echo=FALSE,eval=!exists("mydogU"),fig.width=10,fig.height=8,warning=FALSE,message=FALSE,fig.cap='Partial regression plots for example regions that show omnibus level fwe significant differences from controls.'}
###################
###################
...
#    mydogU$age_BL[mydogU$studyName=='PPMI'] )
# summary(glm( studyNameNum ~ age_BL * commonSex, data=mydogU))
mydogU$TDX[ mydogU$TDX %in% c("MCI-","MCI+")]='MCI'
...
dxdfT1=data.frame()
maxy=2.5
# nmstotest=nmstotest[multigrep( c('T1Hi','trans'),nmstotest,intersect=TRUE)]
...
#  selector = selector & mydogU$subjectID %in% bothids
...
#    print(  knitr::kable( data.frame(mycoffs[-1,coffcols]), "latex", booktabs = TRUE,       caption=shorten_pymm_names(voi)) %>%      kableExtra::kable_styling(latex_options = c("striped", "scale_down")) )
    ...
    mygg0 = jtools::effect_plot( resultsX$full_model,  !!sym('TDX'), interval=T, facet.modx=F ) + ggtitle(paste("X:",voi)) + labs(y = NULL) + theme(legend.position = "none") 
#    mygg1=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=F ) + ggtitle(paste("L:",voi)) + labs(y = NULL) + theme(legend.position = "top") )
    mygg2=( interactions::interact_plot( mdl,  'yearsbl', 'TDX', interval=T, facet.modx=T ) + ggtitle(paste("L:",voi)) + labs(y = NULL) + theme(legend.position = "top") )
...
if ( length( cogglist ) > 0 )
  grid.arrange( grobs = cogglist, nrow=2, top='Interactions between baseline IDPs and MDS-UPDRS changes within PD.' )
###########################################################################
```

```{r quicknnh,echo=FALSE,eval=!exists("qdfsignnh"),fig.width=10,fig.height=6,warning=FALSE,message=FALSE,fig.cap='SiMLR mapping between M3RI and PD symptomology.'}
qdfcols =  c("cog", "voi" , 'n', "anv.x", "anv", 'sig', othercolsnm)
#########################################################
...
for ( voi in (1:npc) ) {
  ...
  #    stadnnh[n,'n.long']=poptbl[2]
      ...
      if ( anvp <= nnhqth | anvpx <= nnhqth ) {
        ...
        grid.arrange( grobs=gglist, nrow=2, top=ttl )
#        print( mycoffs[-1,c(4:5)])
        print(stadnnh[n,] )
    }
  } }
...
qdfsignnh = qdfsignnh[ order( qdfsignnh$anv * qdfsignnh$anv.x ), ]
...
toviz = head( toviz[ , !colnames(toviz) == 'sig'], 50 )
...
qdfsigzNNH=ztable(qdfsignnh,zebra=2,tabular=F)
mycap='Table 7. Significant (fwe) IDP to MDS-UPDRS longitudinal effects for PD+: T1w, DTI and rsfMRI joint prediction.'
