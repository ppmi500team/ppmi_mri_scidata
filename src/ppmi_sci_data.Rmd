...
```{r quicklmcog,echo=FALSE,eval=buildsci,fig.width=9,fig.height=4,warning=FALSE,message=FALSE}
#########################################################
...
for ( cog in  cogs  ) {
  for ( voi in nmstotest ) {
    ...
    cogformcommon=paste( paste0(cog,"_delta"), "~ ",paste0(cog,"_BL"),"+ (1|PATNO) + T1Hier_resnetGrade  + duration_yrs + ",motname," + LEDD + commonEdu + age_BL + commonSex + brainVolume_BL + TDX + ( yearsbl )" )
...
#      print( interactions::interact_plot( mdl, 'yearsbl', modx='TDX',  main.title=ttl, interval=TRUE, plot.points =FALSE, facet.modx=F, int.type = c("confidence", "prediction")[1], partial.residuals=F,mod2=!!sym(voi)  ) )
       print( interactions::interact_plot( mdl, 'yearsbl', mod2='TDX',  main.title=ttl, interval=TRUE, plot.points =FALSE, facet.modx=F, int.type = c("confidence", "prediction")[1], partial.residuals=F,modx=!!sym(voi)  ) )
#      suppressMessages( threewayinteraction(  temp2, 'yearsbl', cogdel, 'TDX', voi, anatshow=ttl3 ) )
